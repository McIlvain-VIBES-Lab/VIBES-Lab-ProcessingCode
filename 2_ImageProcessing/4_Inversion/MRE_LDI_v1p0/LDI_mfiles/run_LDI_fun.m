function outfilename = run_LDI_fun(fitkernel,Delta,smoothfilter,nanfract, LDIdir, inputfile)
...
for Nxy=1:length(fitrange)
    fitsmooth_uvw_rjofun_spiral_R2_parfor(cat(4,real(u),real(v),real(w)),cat(4,imag(u),imag(v),imag(w)),nanmaskfinal,voxelsize*[1 1 1], PC2micron*1e-3, fms, rho, h,Delta,nanfract,fitrange(Nxy), usecurl,curldata,smoothfilter,[],savefilename{Nxy});
end %parfor
%matlabpool close
