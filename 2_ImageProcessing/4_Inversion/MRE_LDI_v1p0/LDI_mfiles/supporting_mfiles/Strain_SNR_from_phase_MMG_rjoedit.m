function [OSS_SNR,Motion_SNR,OSS_SNR_Dist,Motion_SNR_Dist,oss,ons,curldsp,sall,Urf,Uif,MSNR_4D]=Strain_SNR_from_phase_MMG_rjoedit(P,mask,res,filtwidth,ord,Nfit,OSS_SNRfname)
...
% 29 March 2011
%   edited by R. Okamoto Washington Univerity
% minor edits, Feb 2015, Ruth Okamoto
%   added line to avoid use of smooth3dimage; this line was later commented
% out (has little effect on OSS_SNR values). Current version requires smooth3dimage.m provided by Matt McGarr
% adds additional output arguments 
%   curldsp,sall,Urf,Uif. These are calculated in function OSS_calc and are
%   added as output arguments to that function as well
%
%  curldsp is a 4D array with complex values of curl components (x, y, z)
%  sall is a 4D array with complex values of 6 strain components 
%    sall=[sxx syy szz sxy/2 syz/2 sxz/2];  % the divide by 2 is used for
%    consistency with our strains and to make strain tensor components
%  Urf and Urf are filtered displacements from OSS_Calc function below
% minor edits, 11 Mar 2015 - added filename as input variable and save smoothed
%   displacement fields 
% minor edit, 1 June 2015, added MSNR_4D as an output (line 288) and 6 June
% 2015 added MSNR_4D to variables stored in output file
% minor edit, 17 May 2016, changed mean to nanmean in calculation of
% OSS_SNR
...
% OSS_SNRfname = name of file to save data to 
...
Parr=  true; %false %true; % Paralell Matlab indicator
nlabs=7; % Number of Matlab Processes to use

opt=1; % Output Option (1=lots of figs, 2=just OSS, 3=no figures output)

ord_def=2; % Order of polynomial
Nfit_def=3; %Size of fitting block
OSS_SNRfname_def='OSS_SNR.mat';  %default name to save to
...
if(nargin<7)
    OSS_SNRfname=OSS_SNRfname_def; % Size of block to fit data to. Needs to be odd.
    disp(['Using Default Output file name :: ' OSS_SNRfname])
end
if(mod(Nfit,2)==0)
...
%%%  CODE BELOW CHANGED BY Ruth Okamoto to use smooth3 %%%
% for ii=1:3
%     ErrorMap(:,:,:,ii)=smooth3dimage(ErrorMap(:,:,:,ii),0.5,mask); % use a gaussian smoothing filter with SD=0.5
%    %ErrorMap(:,:,:,ii)=smooth3(ErrorMap(:,:,:,ii).*mask,'gaussian',[3 3 3],0.5); % use a gaussian smoothing filter with SD=0.5
% end
%%% END OF CHANGED CODE
...
% OSS of motions
[oss,ons,curldsp,sall,Urf,Uif]=OSS_calc(Ur,Ui,mask,res,filtsiz,filtwidth,ord,Nfit,Parr);
...
% RJO changed next line to nanmean to avoid problems
OSS_SNR=nanmean(oss(Imsk))/nanmean(oss_nse(Imsk));
...
for ii=1:3
    ...
    MSNR_4D(:,:,:,ii,1)= 1/6.*abs(Ur(:,:,:,ii))./abs(Ur_n(:,:,:,ii)); % added by RJO 6/1/2015, updated 6/6/2015
    ...
    MSNR_4D(:,:,:,ii,2)= 1/6.*abs(Ui(:,:,:,ii))./abs(Ui_n(:,:,:,ii)); % added by RJO  6/6/2015
  
end
OSS_SNR_Dist(mask~=1)=0;
% updated by R. Okamoto 3/11/2015 to allow user to specify file name as
% variable and added Urf and Uif as output variables
% updated by R. Okamoto 6/6/2015 to add MSNR_4D as saved output variable

save(OSS_SNRfname,'filtwidth','Nfit','ord','OSS_SNR','Motion_SNR','OSS_SNR_Dist','Motion_SNR_Dist','oss','ons','ossCD','sall','curldsp','Urf','Uif','MSNR_4D');
end % End of Strain SNR calculation

function [oss,ons,curldsp,sall,Ur,Ui]=OSS_calc(Ur,Ui,mask,res,filtsiz,filtwidth,ord,Nfit,Parr)
...
sall=zeros([dim+2*bufsiz 6])+1i*zeros([dim+2*bufsiz 6]);
...
if(Parr)
    parfor ii=1+bufsiz:dim(1)+bufsiz   
        ...
        % solved this problem.
        [oss(ii,:,:),ons(ii,:,:),curldsp(ii,:,:,:),sall(ii,:,:,:)]=process_slice(ii,Ur,Ui,mask,ord,lf,dim,bufsiz,ntrms,res);       
    end
else
...
junk=sall;
sall=zeros([dim 3])+1i*zeros([dim 3]);
for ii=1:6,
    sall(:,:,:,ii)=zerobuffer3d(junk(:,:,:,ii),bufsiz,'remove');
end
end


function [oss_ii,ons_ii,curldsp_ii,sall_ii]=process_slice(ii,Ur,Ui,mask,ord,lf,dim,bufsiz,ntrms,res)
...
for jj=1+bufsiz:dim(2)+bufsiz
    for kk=1+bufsiz:dim(3)+bufsiz
        if mask(ii,jj,kk)==1
            ...
            [oss_ii(jj,kk),ons_ii(jj,kk),curldsp_ii(jj,kk,:),sall_ii(jj,kk,:)]=octshearstain_curl(ar,ai,dx,dy,dz);
        end
    end
end
...
function [ossval,onsval,curlu,sall]=octshearstain_curl(ar,ai,dx,dy,dz)
...
sxz=dudz+dwdx;
syz=dvdz+dwdy;
sall=[sxx syy szz sxy/2 syz/2 sxz/2];
...
%                 wpy=W(ii,jj+1,kk);
%                 wmy=W(ii,jj-1,kk);
% these lines were changed by RJO to be consistent with meshgrid used
% elsewhere in code, % note y is assumed to be increasing w/ row
                upy=U(ii+1,jj,kk);
                umy=U(ii-1,jj,kk);
                vpy=V(ii+1,jj,kk);
                vmy=V(ii-1,jj,kk);
