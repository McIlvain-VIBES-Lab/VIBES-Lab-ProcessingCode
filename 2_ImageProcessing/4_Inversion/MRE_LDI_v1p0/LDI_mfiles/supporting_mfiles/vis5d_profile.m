%%vis5d_profile(seg, mycmap, figName, mycaxis,DimLabels,fsize): 
%%a GUI-enabled 3D visualization of array corresponding to the 3D, 4D or 5D volume 
% based on vis3d from author below
% Ruth Okamoto made numerous changes/additions as documented below
%{
Joshua Stough
JHU, iacl
June 2012
April 2013

BSD Licence:
Copyright (c) 2013, Joshua Stough
All rights reserved.

Please comment on MATLAB fileexchange if you find this useful. Thank you.

This program offers a visualization of 3D volumes.

Obviously, this is not a new idea. This program might offer over other
programs:
    -change slice via click drag or scroll
    -orthogonal planes are visualized on each projection in a color-coded
    manner, allowing a better sense of where you are in the 3D space.
    -windowing capability
    -changing the colormap
    -neurological coordinate frame

However, after seeing the slice function, I think I must be missing a lot
of built-in functionality, making this program pointless. But I share in any
event as I obviously cannot see all ends (nerd).

References to really good and thoughtful 3D viewer implementations in
matlab (including buttons and all axis flipping that I can't do):
http://www.mathworks.com/matlabcentral/fileexchange/2256-orthoview
http://www.mathworks.com/matlabcentral/fileexchange/764-sliceomatic

For some 3d matrix of double or uint8, call
>> vis3d(img);
or, if you have some multilabel segmentation image, try:
>> vis3d(seg, 'multi');
or, if you want define your own colormap, you can send that instead:
>> mymap = colormap('lines');
>> vis3d(seg, mymap);
lastly, you can customize the figure title:
>> vis3d(seg, '', 'My Figure Title');

Updates/Modifications:
4/13: 
-fixed slice number issue: the out-of-plane slice number was incorrect 
in the initial window.
-changed interaction: now, scrolling changes slice number 
-fixed the 3d volume visualization errors arising from the treatment of 
images versus matrices in matlab.
-fixed tick mark misrepresentation.
-added windowing on the colormap
-added choice of colormap in addition to typical maps.

- RJO changes in 
- 9/2015
RJO customized/modified the original code to handle 5D MRE human datasets
changed what is plotted in subplot(2,2,4) to be the time data
- Changed viewing directions and axes labels
- added sliders and boxes for controlling timepoints (array dir 4) and
displacement directions (array dim 5)
- added input argument for default value for image color mapping (mycaxis)
- 3/2016
- RJO working on Matlab R2014b graphics handle compatibility
-chanegd how whichView variable is assigned per info in fileExchange
-7/2016
- RJO customized to create profile views
- changed color scheme for slices to be consistent (red=RL, green=AP, blue
= FH for human data)
-10/2016
- RJO added functionality to label 4th and 5th dimensions appopriately
- and to jump to a cross hair location
-11/2016
-RJO renamed file to better reflect its purpose
%}

                            
function [mainHandle] = vis5d_profile(seg, mycmap, figName, mycaxis,DimLabels,fsize)

    %We check for the colormap after we have a figure open.
    if nargin < 3
        figName = 'vis3d: click and scroll/drag vertically (up=closer to eye).'; 
        userSpecFigName = false; %did the user specify the figure name.
    else
        userSpecFigName = true;
    end
    %arg 2 taken care of below.


    if ~(isa(seg, 'double') || isa(seg, 'uint8'))
        fprintf('The image is not double or uint8.  You may receive many warnings.\n');
        fprintf('Cast your image to either before sending here.');
    end
    [s1,s2,s3,s4,s5]=size(seg);
    cur4=1;  % set initial time point to 1
    cur5=1;  % set initial direction to 1
%     if s4 >1,  % then image is 4D
% %        fullseg=seg;
%         cur4 = 1;
%         if s5 > 1% then image is 5D
%               cur5 = 1;
%   %            seg = seg(1:s1,1:s2,1:s3,cur4,cur5);
%         else
%             seg=seg(1:s1,1:s2,1:s3,cur4);
%         end % if s5
%     end % if s4
            
   %seg = seg/max(seg(:)); %Make 0-1. 
   if nargin >=4,
       segRange=mycaxis;
   else
    segRange = [min(seg(:)) max(seg(:))];
    origSegRange = segRange;
   end
   
   if nargin <6, 
       voxspace=[1 1 1]; % uniform voxel spacing
   else
       if ~length(voxspace)==3,
           error('voxel spacing vector must have 3 elements');
       end
   end
   
    %segRange is required to ensure that if you view a slice with more or
    %less range, you get colors consistent with other slices.
    
    sizeSeg = size(seg);
    sn = round(sizeSeg/2);
    %slice numbers for the three projections.
    
    titleLines = {'Axial view: ', 'Cor view: ', 'Sag view: '};
    titleColors = {'r', 'g', 'b'};
    %titleColors = {[.3 .3 .8], [.8 .3 .3], [.3 .8 .3]};
    if nargin <5,
        DimLabels={'Time Pt','Dir'};
    end
    if nargin < 6,
        fsize=[1400 800];
    end
    %The figure handle, with its callbacks.
    mainHandle = figure('Position', [50 50 fsize(1) fsize(2)], ...
        'WindowButtonDownFcn', @buttonDownCallback, ...
        'WindowButtonUpFcn', @buttonUpCallback, ...
        'Name', figName,'Color','w'); hold on;
    
    
   
    
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Adding colormap popupmenu
    colormapText = uicontrol('Style', 'text', 'Position', [fsize(1)*0.7 20 100 25], ...
        'FontSize', 12, 'FontWeight', 'bold');
    set(colormapText, 'String', 'Colormap:', 'ForegroundColor', [1 0 0]);
    
    
    colormapPopup = uicontrol('Style', 'popupmenu', ...
        'Position', [fsize(1)*0.8 25 200 20], ...
        'Callback', @specifyColormap);
    set(colormapPopup, 'String', {'gray'; 'multi-label'; 'jet'}, ...
        'Value', 1, 'FontSize', 12, 'FontWeight', 'bold');
    
    %define the colormaps.
    %A colormap I find useful for multi-label segmentation images, which is
    %why I made this thing to begin with (and why the image is called seg).
    multi = [0 0 0;
            1 0 1;
            0 .7 1;
            1 0 0;
            .3 1 0;
            0 0 1;
            1 1 1;
            1 .7 0];
    graymap = colormap('gray');
    jetmap = colormap('jet');
    the_cmaps = {graymap; multi; jetmap};
    
    %Is there a user-defined map?
    %Now that we're sure to have a figure open, get a colormap.
    if nargin < 2 || strcmp(mycmap, '')
        mycmap = gray;
    elseif strcmp(mycmap, 'multi')
        mycmap = multi;
        set(colormapPopup, 'Value', 2);
    else
        %User defined a colormap to use.  Set it as current.
        the_cmaps{end+1} =  mycmap;
        set(colormapPopup, 'String', {'gray'; 'multi-label'; 'jet'; 'user-spec'}, ...
        'Value', 4);
    end
    %End of colormap popup menu.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
   
    %Now, we plot the three orthogonal slices and try to get the two lines
    %on each slice, which represent the other planes.
    %Notice we are using axis xy, so that the tick marks are correct, that
    %also means we don't need to flip z in yz and xz views...
   % handles{1} = subplot(2,2,3); 
   handles{1}=subplot('Position',[0.1 0.65 0.2 0.35]); 
   imHandles{1} = imagesc(voxspace(1)*(1:size(seg,2)),voxspace(2)*(1:size(seg,1)),squeeze(seg(:,:,sn(3),cur4,cur5)), segRange); colormap(mycmap); axis image;  %axis xy
   ylabel('Post-Ant');xlabel('Right-Left')
    %set(handles{1}, 'LooseInset', get(gca,'TightInset')) %made smaller...
    titleHandles{1} = title(handles{1}, 'Axial view', 'Color', titleColors{3}, 'FontSize', 14, 'FontWeight', 'bold');
    colorbar('eastoutside');

    %    handles{2} = subplot(2,2,1); 
    handles{2}=subplot('Position',[0.4 0.75 0.2 0.2]);
    imHandles{2} = imagesc(voxspace(1)*(1:size(seg,2)),voxspace(3)*(1:size(seg,3)),squeeze(permute(seg(sn(1),:,:,cur4,cur5),[3 2 1 4 5])), segRange); colormap(mycmap); axis image; axis xy;
    titleHandles{2} = title(handles{2}, 'Cor view', 'Color', titleColors{2}, 'FontSize', 14, 'FontWeight', 'bold');
    ylabel('Foot-Head');xlabel('Right-Left')
 
    %set(handles{2}, 'XDir', 'rev'); 
    %I used reverse here to have appropriate neurological coord, but then I
    %thought, people like to see 'through the eyes of the patient' in
    %coronal (i.e., patient l-r equals l-r in the image).
    
   
    handles{3}=subplot('Position',[0.7 0.75 0.2 0.2]);
    imHandles{3} = imagesc(voxspace(2)*(1:size(seg,1)),voxspace(3)*(1:size(seg,3)),squeeze(permute(seg(:,sn(2),:,cur4,cur5),[3 1 2 4 5])), segRange); colormap(mycmap); axis image; axis xy;
    titleHandles{3} = title(handles{3}, 'Sag view', 'Color', titleColors{1}, 'FontSize', 14, 'FontWeight', 'demi');
    ylabel('Foot-Head');xlabel('Ant-Post')
    
    
    %Making sure the titles all have the initial slice number as well.
    offsliceCoord = [3 1 2];
%     for i = 1:3
%         set(titleHandles{i}, 'String', sprintf('%s%03d', titleLines{i}, sn(offsliceCoord(i))));
%     end
            for k=1:3,
            set(titleHandles{k}, 'String', ...
            sprintf('%s%03d %s%s%02d %s%s%02d', titleLines{k}, sn(offsliceCoord(k)),DimLabels{1},': ',cur4,DimLabels{2},': ',cur5));
        end

    
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %lower and upper bound on the intensity window and text box.
    
    %Adding uicontrol elements would remove the toolbar from the figure
    %without this statement.
    set(mainHandle, 'Toolbar', 'figure');
    
    IntWinSliderHandle{1} = uicontrol('Style', 'slider', ...
        'Position', [fsize(1)*0.8 fsize(2)/10 fsize(1)*0.1 20], ...
        'Callback', @IntWinBound);
    set(IntWinSliderHandle{1}, 'Value', segRange(1));
    
    IntWinSliderHandle{2} = uicontrol('Style', 'slider', ...
        'Position', [fsize(1)*0.8 (fsize(2)/10)+25 200 20], ...
        'Callback', @IntWinBound);
    set(IntWinSliderHandle{2}, 'Value', segRange(2));
    
    
    for i = 1:2
        set(IntWinSliderHandle{i}, 'Min', segRange(1));
        set(IntWinSliderHandle{i}, 'Max', segRange(2));
    end
    
    IntWinTextBound{1} = uicontrol('Style', 'edit', ...
        'Position', [fsize(1)*0.9 fsize(2)/10 60 20], ...
        'Callback', @defineBound);
    set(IntWinTextBound{1}, 'String', segRange(1));
    
    IntWinTextBound{2} = uicontrol('Style', 'edit', ...
        'Position', [fsize(1)*0.9 (fsize(2)/10)+25 60 20], ...
        'Callback', @defineBound);
    set(IntWinTextBound{2}, 'String', segRange(2));
    
    IntWinText = uicontrol('Style', 'text', 'Position', [fsize(1)*0.7 fsize(2)/10 100 25], ...
        'FontSize', 12, 'FontWeight', 'bold');
    
    set(IntWinText, 'String', 'Windowing', ...
            'ForegroundColor', [1 0 0]);
    %End of intensity windowing elements.

    % set up sliders for time point
    
        IntDim4SliderHandle{1} = uicontrol('Style', 'slider', ...
        'Position', [fsize(1)*0.8 fsize(2)*0.3 fsize(1)*0.1 20], ...
        'Callback', @IntDim4);
        set(IntDim4SliderHandle{1}, 'Min',1,'Max',s4);
        set(IntDim4SliderHandle{1}, 'Value', cur4);
   
        IntTextDim4Bound{1} = uicontrol('Style', 'edit', ...
        'Position', [fsize(1)*0.9 fsize(2)*0.3 60 20], ...
        'Callback', @defineDim4);
        set(IntTextDim4Bound{1}, 'String', cur4);

        IntDim4Text = uicontrol('Style', 'edit', 'Position', [fsize(1)*0.7 fsize(2)*0.3 100 25], ...
            'FontSize', 12, 'FontWeight', 'bold','Callback',@chgDim4Text);
    
        set(IntDim4Text, 'String', DimLabels{1}, ...
            'ForegroundColor', [0 0 1]);

   % set up slider for direction
        IntDim5SliderHandle{1} = uicontrol('Style', 'slider', ...
        'Position', [fsize(1)*0.8 fsize(2)*0.2 fsize(1)*0.1  25], ...
        'Callback', @IntDim5);
        set(IntDim5SliderHandle{1}, 'Min',1,'Max',s5);
        set(IntDim5SliderHandle{1}, 'Value', cur5);
      
        
        IntDim5TextBound{1} = uicontrol('Style', 'edit', ...
        'Position', [fsize(1)*0.9 fsize(2)*0.2 60 20], ...
        'Callback', @defineDim5);
        
        set(IntDim5TextBound{1}, 'String', int2str(cur5));
        
        IntDim5Text = uicontrol('Style', 'edit', 'Position', [fsize(1)*0.7 fsize(2)*0.2   100 25], ...
            'FontSize', 12, 'FontWeight', 'bold','Callback',@chgDim5Text);
    
        set(IntDim5Text, 'String', DimLabels{2}, ...
            'ForegroundColor', [0 0 1]);
    
        % set up boxes for crosshair position
        CrossHairText = uicontrol('Style', 'text', 'Position', [fsize(1)*0.7 fsize(2)*0.25   125 25], ...
            'FontSize', 12, 'FontWeight', 'bold');
    
        set(CrossHairText, 'String', 'CrossHairLoc', ...
            'ForegroundColor', [1 0 0]);
        
       CH3val = uicontrol('Style','edit',...
                'String',sn(3),...
                'Max',1,'Min',0,...
                'Position',[fsize(1)*0.8 fsize(2)*0.25   25 25],'Callback',@defineCH3);
       CH1val = uicontrol('Style','edit',...
                'String',sn(1),...
                'Max',1,'Min',0,...
                'Position',[fsize(1)*0.82 fsize(2)*0.25   25 25],'Callback',@defineCH1);
       CH2val = uicontrol('Style','edit',...
                'String',sn(2),...
                'Max',1,'Min',0,...
                'Position',[fsize(1)*0.84 fsize(2)*0.25   25 25],'Callback',@defineCH2);

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    
    
    
    
    
    
    
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %And the 3D view goes at the bottom
     handles{4}=subplot('Position',[0.1 0.1 0.5 0.2]);   
           hslc=plot(squeeze(seg(sn(1),sn(2), sn(3),:,:)));
           ylabel('Voxel values');xlabel(IntDim4Text.String)
        for p=1:size(seg,5),
            M{p}=[IntDim5Text.String,' ',int2str(p)];
        end %for
        legend(M(1:size(seg,5)),'Location','bestoutside')
        titleHandles{4} =title(['Vox AP=',int2str(sn(1)),' RL=', int2str(sn(2)),' FH=',int2str(sn(3))]);        

    %End of the 3D view elements
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %And the first sliced view
        handles{5}=subplot('Position',[0.1 0.4 0.2 0.2]);
        hslc=plot(voxspace(1)*(1:size(seg,2)),squeeze(seg(sn(1),:, sn(3),cur4,cur5)),titleColors{1});
        ylabel('Voxel values');xlabel('Right-Left')
%         M=strrep(strrep('tp #, dir $','#',int2str(cur4)),'$',int2str(cur5));
%         legend(M,'Location','best')
         titleHandles{5} =title([' Vox AP=', int2str(sn(1)),' FH=',int2str(sn(3)),', ',DimLabels{1},' ',int2str(cur4),DimLabels{2},' ',int2str(cur5)]);

    %End of the first slice view elements
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %And the  second sliced view
        handles{6}=subplot('Position',[0.4 0.4 0.2 0.2]);
        hslc=plot(voxspace(3)*(1:size(seg,3)),squeeze(seg(sn(1),sn(2),:,cur4,cur5)),titleColors{3});
        ylabel('Voxel values');xlabel('Foot-Head')
%         M=strrep(strrep('tp #, dir $','#',int2str(cur4)),'$',int2str(cur5));
%         legend(M,'Location','best')
        titleHandles{6} =title([' Vox RL=', int2str(sn(2)),' AP=',int2str(sn(1)),', ',DimLabels{1},' ',int2str(cur4),DimLabels{2},' ',int2str(cur5)]);

    %End of the second slice view elements
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %And the third sliced view
        handles{7}=subplot('Position',[0.7 0.4 0.2 0.2]);
        hslc=plot(voxspace(2)*(1:size(seg,1)),squeeze(seg(:,sn(2), sn(3),cur4,cur5)),titleColors{2});
        ylabel('Voxel values');xlabel('Ant-Post')
%         M=strrep(strrep('tp #, dir $','#',int2str(cur4)),'$',int2str(cur5));
%         legend(M,'Location','best')
        titleHandles{7} =title([' Vox RL=', int2str(sn(2)),' FH=',int2str(sn(3)),', ',DimLabels{1},' ',int2str(cur4),DimLabels{2},' ',int2str(cur5)]);

        %End of the 3D view elements
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
    
    
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Generate the lines representing the orthogonal planes.  This part is 
    %so confusing, in part because line will use the bottom
    %left, whereas we're thinking of the top left (image) as being 0,0.
    %But it's mostly my fault, because coordinate systems are so HARD...
    
    lines{1}.x = voxspace(1)*[1 sizeSeg(2); sn(2) sn(2)]'; 
    lines{1}.y = voxspace(2)*[sn(1) sn(1); 1 sizeSeg(1)]';
    %the x coords of the horiz and vertical line respectively.
    %the y coords of the horiz and vertical line respectively.
    
    lines{2}.x = voxspace(1)*[1 sizeSeg(2); sn(2) sn(2)]';
    lines{2}.y = voxspace(3)*[sn(3) sn(3); 1 sizeSeg(3)]';
    
    lines{3}.x = voxspace(2)*[1 sizeSeg(1); sn(1) sn(1)]';
    lines{3}.y = voxspace(3)*[sn(3) sn(3); 1 sizeSeg(3)]';
    
    %We now instantiate the lines.  We also want the line colors to reflect
    %the plane they represent, which is given by the titleColors. The trick
    %is weird though: when we're visualizing the XY plane, the two lines on
    %it are the XZ plane (vertical), and the YZ plane, horizontal.  whew.
    %for each, first is horizontal, next is vertical.
    lineHandles = {};
    subplot(handles{1}); % axial view
    lineHandles{1}(1) = line(lines{1}.x(:,1), lines{1}.y(:,1), 'Color', titleColors{1}, 'LineWidth', 1);
    lineHandles{1}(2) = line(lines{1}.x(:,2), lines{1}.y(:,2), 'Color', titleColors{2}, 'LineWidth', 1);
    
    subplot(handles{2}); %coronal view 
    lineHandles{2}(1) = line(lines{2}.x(:,1), lines{2}.y(:,1), 'Color', titleColors{1}, 'LineWidth', 1);
    lineHandles{2}(2) = line(lines{2}.x(:,2), lines{2}.y(:,2), 'Color', titleColors{3}, 'LineWidth', 1);
    
    subplot(handles{3}); % sagittal view
    lineHandles{3}(1) = line(lines{3}.x(:,1), lines{3}.y(:,1), 'Color', titleColors{2}, 'LineWidth', 1);
    lineHandles{3}(2) = line(lines{3}.x(:,2), lines{3}.y(:,2), 'Color', titleColors{3}, 'LineWidth', 1);
    %End of the lines 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    
       
    %Variables to keep track of mouse motion and which slice is being
    %maniupulated. 
    lastPoint = [];
    whichView = 1;
    
    %At this point, time to define our callbacks.
    
    %This is the callback when the user clicks.  We want to determine which
    %if any projection they clicked on so that further vertical motion can
    %be associated with changing the slice for that projection. 
    function buttonDownCallback(varargin)
        
        p = get(gca, 'CurrentPoint');
        lastPoint = [p(1); p(3)];
        
        %We know where they clicked with respect to the last active axes,
        %determine which axes that was.
        % whichView = find(cell2mat(handles) == gca);
        % line above commented out and 
        % next 4 lines suggested by original author to fix compatibility w
        % R2014b
        for i = 1:max(size(handles)) 
            if(handles{i}==gca) whichView =i; 
            end 
        end
        
        %  end of new code
        
        if whichView >= 4 
            %Can't do anthing with the 3D view, unless it's rotate
            %selection.  The user has to do it themselves.
            %set(rotHandle, 'Enable', 'on', 'ActionPostCallback', @buttonUpOnRotateCallback);
            return
        else
            if whichView == 1,
                ViewVoxSpace = [voxspace(1) voxspace(2)];
            else
                if whichView==2,
                    ViewVoxSpace = [ voxspace(1) voxspace(3)];
                else % whichView = 4
                    ViewVoxSpace = [ voxspace(2) voxspace(3)];
                end
            end
        end
        
        %By getting the camera position on a click, we can maintain the 3d
        %perspective in the case of changing slices.
        camPos3D = get(handles{4}, 'CameraPosition');
        
        %Now determine if they clicked on a valid spot, within the coords
        %of the chosen image/axes.
        xlim = get(gca, 'XLim');
        ylim = get(gca, 'YLim');
        if lastPoint(1) >= xlim(1) && lastPoint(1) <= xlim(2) && ...
                lastPoint(2) >= ylim(1) && lastPoint(2) <= ylim(2)
            %They clicked on a valid point, time to activate the motion
            %callback. Also let's tell them the intensity at the point they
            %click. But we also don't want to remove what they decided to
            %call the main figure. 
            t_img = get(imHandles{whichView}, 'CData');
            p=round(p);
        %   p([1 3]) = round(p([1 3])./ViewVoxSpace); % this is tricky
        %    lastPoint=lastPoint./ViewVoxSpace(:);  % this is Ruth's kludge to keep things working
            if userSpecFigName
                set(mainHandle, 'Name', sprintf('%s: Pos: %d %d, Int: %f', ...
                    figName, p(3), p(1), t_img(p(3), p(1))));
            else
                set(mainHandle, 'Name', sprintf('Pos: %d %d, Int: %f', ...
                    p(3), p(1), t_img(p(3), p(1))));
            end
            set(mainHandle, 'WindowButtonMotionFcn', @dragCallback);
            set(mainHandle, 'WindowScrollWheelFcn', @scrollCallback);
            %fprintf('You clicked on %f, %f on gca %f (%d).\n', p(1), p(3), gca, whichView);
        %else
            %fprintf('Oops, you did''t click flippedx(sn(1))a valid point.\n');
        end
    end

    %At this point, it's established that the user clicked on one of the
    %images and is currently dragging.  We will change the image on the
    %relevant slice and update slice info (in the title). 
    %And then, if this works, I'll try the red lines on the other two
    %projections.
    function dragCallback(varargin)
        p = get(gca, 'CurrentPoint');
        motionV = round([p(1); p(3)])- lastPoint;
        if abs(motionV(2)) < 2
            return
        end %Wait until the motion is noticeable.
        
        lastPoint = [p(1); p(3)];
        
        
        newslice = sn(offsliceCoord(whichView)) + round(motionV(2)/2);
        
        updateSliceViz(newslice);
  
    end

    function buttonUpCallback(varargin)
        %Nullify the motion and correct the 3D view.
        %Slice is too easy though, thanks Matlab.  I should be ashamed
        %http://www.mathworks.com/matlabcentral/fileexchange/2256-orthoview
        set(mainHandle, 'WindowButtonMotionFcn', '');
        ReplotAll(varargin)
        set(CH3val,'String',int2str(sn(3)));
        set(CH1val,'String',int2str(sn(1)));        
        set(CH2val,'String',int2str(sn(2)));


%         axes(handles{4}); 
% %        if exist('fullseg','var'),
%                     hslc=plot(squeeze(seg(sn(1),sn(2), sn(3),:,:)));
%         axis([1 size(seg,4) segRange(1) segRange(2)]);
%         % else % don't change x axis
%         %     cax=axis; axis([cax(1) cax(2) segRange(1) segRange(2)]);
%         
%             ylabel('Voxel values');xlabel('Time pts')
%             M={'dir 1';'dir 2';'dir 3'};
%             legend(M(1:size(seg,5)),'Location','BestOutside')
%             title(['Vox AP=',int2str(sn(1)),' RL=', int2str(sn(2)),' FH=',int2str(sn(3))]);  
% 
%         axes(handles{5});
%             kid=get(gca,'Children');
%             set(kid(1),'YData',squeeze(seg(sn(1),:,sn(3),cur4,cur5)));
%             M=strrep(strrep('tp #, dir $','#',int2str(cur4)),'$',int2str(cur5));
%             legend(M,'Location','best')
%             title([' Vox AP=', int2str(sn(1)),' FH=',int2str(sn(3))]); 
%             axis([1 size(seg,2) segRange(1) segRange(2)]);
% 
% 
%    
%         axes(handles{6});
%             kid=get(gca,'Children');
%             set(kid(1),'YData',squeeze(seg(sn(1),sn(2),:,cur4,cur5)));
%             M=strrep(strrep('tp #, dir $','#',int2str(cur4)),'$',int2str(cur5));
%             legend(M,'Location','best')
%             title([' Vox RL=', int2str(sn(2)),' AP=',int2str(sn(1))]);        
%             axis([1 size(seg,3) segRange(1) segRange(2)]);
% 
%         axes(handles{7});
%             kid=get(gca,'Children');
%             set(kid(1),'YData',squeeze(seg(:,sn(2),sn(3),cur4,cur5)));
%             M=strrep(strrep('tp #, dir $','#',int2str(cur4)),'$',int2str(cur5));
%             legend(M,'Location','best')
%             title([' Vox RL=', int2str(sn(2)),' FH=',int2str(sn(3))]);        
%             axis([1 size(seg,1) segRange(1) segRange(2)]);


% hslc=slice(visSeg, flippedx(sn(1)), sn(2), sn(3));%rotate3d on;
%         axis equal; axis vis3d; set(hslc(1:3),'LineStyle','none');
%         xlabel 'p-a' ;ylabel 'l-r' ;zlabel 'i-s';
%         
%         %Same stupid and incorrect label issue.
%         myx = get(gca, 'XTickLabel');
%         set(gca, 'XTickLabel', flippedx(str2num(myx)));
%     
%         title 'Manually enable/disable Rotate3D.'
%         set(handles{4}, 'CameraPosition', camPos3D);
%         end
    end

    %I had problems with 'ActionPostCallback' and so I'm leaving rotation
    %of the 3d plot up to the user clicking on and off the button.
    %function buttonUpOnRotateCallback(varargin)
    %    set(rotHandle, 'ActionPostCallback', '');
    %    set(rotHandle, 'Enable', 'off');
    %end
    
    function scrollCallback(src,evnt)
        %Change whichView's slice by the vertical motion.
        offsliceCoord = [3 1 2];
        %if they're moving xy slice, that's a change in z, etc.
        
        if whichView == 4
            return
        end
        
        if evnt.VerticalScrollCount > 0
            newslice = sn(offsliceCoord(whichView)) - 1;
        else
            newslice = sn(offsliceCoord(whichView)) + 1;
        end
        
        updateSliceViz(newslice);
        
    end


    function updateSliceViz(newslice)
        %Change whichView's slice by the vertical motion.
        
        offsliceCoord = [3 1 2];
        %if they're moving xy slice, that's a change in z, etc.
        
        
        if newslice > 0 && newslice <= sizeSeg(offsliceCoord(whichView))
            sn(offsliceCoord(whichView)) = newslice;
        end
        subplot(handles{whichView});
        
        %This is a tough to comprehend part.  I want to move the lines
        %representing the current plane in the other orthogonal views.
        %I'll be changing line properties accordingly.
        if whichView == 1
            %XY view (axial)
            %imagesc(squeeze(seg(:,:,sn(3))), segRange); axis image; hold on
            %While you don't have to respecify the colormap, you do need to
            %remind imagesc of the appropriate range and ensure the axes
            %are good. In fact, I forgot that with matlab, everything is an
            %object.  Just change the data in the object.
            set(imHandles{1}, 'CData', squeeze(seg(:,:,sn(3),cur4,cur5)));
            
            %Moving in z, so change the horizontal lines in the YZ and XZ
            %plots.
            set(lineHandles{2}(1), 'YData', [sn(3) sn(3)]');
            set(lineHandles{3}(1), 'YData', [sn(3) sn(3)]');
            
        elseif whichView == 2
            %YZ view (coronal)
            set(imHandles{2}, 'CData', squeeze(permute(seg(sn(1),:,:,cur4,cur5),[3 2 1 4 5])));
            %imagesc(squeeze(seg(sn(1),:,:)), segRange); axis image
            
            %Moving in x, so change the horizontal line in the XY 
            %(representing front-back) and the vertical in and XZ.
            %plots
            set(lineHandles{1}(1), 'YData', [sn(1) sn(1)]');
            set(lineHandles{3}(2), 'XData', [sn(1) sn(1)]');
        else
            %XZ view (sagittal)
            set(imHandles{3}, 'CData', squeeze(permute(seg(:,sn(2),:,cur4,cur5),[3 1 2 4 5])));
            %imagesc(squeeze(seg(:,sn(2),:)), segRange); axis image 
            
            %Moving in y, so change the vertical line in plot 1 and
            %horizontal line in plot 2.
            set(lineHandles{1}(2), 'XData', [sn(2) sn(2)]');
            set(lineHandles{2}(2), 'XData', [sn(2) sn(2)]');
        end
        %title(handles{whichView}, strcat(titleLines{whichView}, num2str(sn(offsliceCoord(whichView)))),...
        %    'Color', titleColors{whichView});
        %Why recreate the title if all we really want to do is change the
        %string.
        for k=1:3,
            set(titleHandles{k}, 'String', ...
                sprintf('%s%03d %s%s%02d %s%s%02d', titleLines{k}, sn(offsliceCoord(k)),DimLabels{1},': ',cur4,DimLabels{2},': ',cur5));
        end

    end





    function IntWinBound(varargin)
        %Here we perform the linear intensity windowing on the images.
        %First, determine the window selected by the user and update the
        %text appropriately.
        
        for whichSlider = 1:2
            slider_value = get(IntWinSliderHandle{whichSlider}, 'Value');
            segRange(whichSlider) = slider_value;
            set(IntWinTextBound{whichSlider}, 'String', segRange(whichSlider));
        end
        

        %Now, change the CLim of the four axes.  I couldn't find how to get
        %at just the axes (which have CLim defined), because subplot
        %apparently never returned a handle for my handles cell array.
        %Anyway, looping through the main figure's children did the trick.
        %(For now anyway, until I start using uipanels and such).
        for c = get(mainHandle, 'Children')'
            try
                set(c, 'CLim', segRange);
            catch exception
                continue
            end
        end
    ReplotAll(varargin)    
    end

    function defineBound(varargin)
        for whichBound = 1:2
            bound_str = get(IntWinTextBound{whichBound},'String');
            if strcmp(bound_str, 'max')
                bound_value = max(origSegRange);
            elseif strcmp(bound_str, 'min')
                bound_value = min(origSegRange);
            else
                bound_value = str2double(bound_str);
            end
            set(IntWinTextBound{whichBound},'String', num2str(bound_value));
            segRange(whichBound) = bound_value;
        end
        
        for c = get(mainHandle, 'Children')'
            try
                set(c, 'CLim', segRange);
            catch exception
                continue
            end
        end
        ReplotAll(varargin) 
    end

function IntDim4(varargin)
        % Change the time point that is plotted
        
        slider_value = get(IntDim4SliderHandle{1}, 'Value');
        cur4 = round(slider_value);
        %set(IntDim4SliderHandle{1}, 'String', int2str(cur4));
        set(IntTextDim4Bound{1}, 'String', int2str(cur4));
        ReplotAll(varargin);

%         set(imHandles{1},'CData',seg(:,:,sn(3),cur4,cur5));
%         set(imHandles{2},'CData',permute(seg(sn(1),:,:,cur4,cur5),[3 2 1 4 5]));
%         set(imHandles{3},'CData',permute(seg(:,sn(2),:,cur4,cur5),[3 1 2 4 5]));
        
    end
        
 
       

function defineDim4(varargin)
            Dim4_str = get(IntTextDim4Bound{1},'String');
            if strcmp(Dim4_str, 'max')
                Dim4_value = s4;
            elseif strcmp(Dim4_str, 'min')
                Dim4_value = 1;
            else
                Dim4_value = str2double(Dim4_str);
            end
            set(IntTextDim4Bound{1},'String', num2str(Dim4_value));
            cur4 = Dim4_value;
            ReplotAll(varargin);
%             set(imHandles{1},'CData',seg(:,:,sn(3),cur4,cur5));
%             set(imHandles{2},'CData',permute(seg(sn(1),:,:,cur4,cur5),[3 2 1 4 5]));
%             set(imHandles{3},'CData',permute(seg(:,sn(2),:,cur4,cur5),[3 1 2 4 5]));
%             for k=1:3,
%                 set(titleHandles{k}, 'String', ...
%                     sprintf('%s%03d%s%02d%s%02d', titleLines{k}, sn(offsliceCoord(k)),' tp: ',cur4,' Dir: ',cur5));
%             end
%             
%         for c = get(mainHandle, 'Children')'
%             try
%                 set(c, 'CLim', );
%             catch exception
%                 continue
%             end
%         end
end

    function chgDim4Text(varargin)
        Dim4_str = get(IntDim4Text,'String');
        DimLabels{1}=Dim4_str;
    end
    function chgDim5Text(varargin)
        Dim5_str = get(IntDim5Text,'String');
        DimLabels{2}=Dim5_str;
    end
    function IntDim5(varargin)
        % Change the direction that is plotted
        slider_value = get(IntDim5SliderHandle{1}, 'Value');
        cur5 = round(slider_value);
        set(IntDim5SliderHandle{1}, 'String', int2str(cur5));
        set(IntDim5TextBound{1}, 'String', int2str(cur5));
        ReplotAll(varargin)
        
    end

 
       

function defineDim5(varargin)
            Dim5_str = get(IntDim5TextBound{1},'String');
            if strcmp(Dim5_str, 'max')
                Dim5_value = s5;
            elseif strcmp(Dim5_str, 'min')
                Dim5_value = 1;
            else
                Dim5_value = str2double(Dim5_str);
            end
            set(IntDim5TextBound{1},'String', num2str(Dim5_value));
            cur5 = Dim5_value;
            ReplotAll(varargin)
    end


    function specifyColormap(hObject, ~)
        choice = get(hObject, 'Value');
        
        colormap(the_cmaps{choice});
        
        
    end

    function ReplotAll(varargin)
        % Updates all window plots after changes to slider, text entry, or
        % crosshairs movement
        
        set(imHandles{1},'CData',seg(:,:,sn(3),cur4,cur5));
        set(imHandles{2},'CData',permute(seg(sn(1),:,:,cur4,cur5),[3 2 1 4 5]));
        set(imHandles{3},'CData',permute(seg(:,sn(2),:,cur4,cur5),[3 1 2 4 5]));
        for k=1:3,
            set(titleHandles{k}, 'String', ...
            sprintf('%s%03d %s%s%02d %s%s%02d', titleLines{k}, sn(offsliceCoord(k)),DimLabels{1},': ',cur4,DimLabels{2},': ',cur5));
        end
        
         % axes(handles{4}); 
%        if exist('fullseg','var'),
        kid=get(handles{4},'Children');
        for k=1:length(kid),
            set(kid(k),'YData',squeeze(seg(sn(1),sn(2), sn(3),:,(length(kid)+1)-k)));
        end
        if size(seg,4)>1,
        set(handles{4},'XLim',[1 size(seg,4)],'YLim',[ segRange(1) segRange(2)]);
        % else % don't change x axis
        %     cax=axis; axis([cax(1) cax(2) segRange(1) segRange(2)]);
        set(titleHandles{4},'String',['Vox AP=',int2str(sn(1)),' RL=', int2str(sn(2)),' FH=',int2str(sn(3))]);  
        end
        %axes(handles{5});
            kid=get(handles{5},'Children');
            set(kid(1),'YData',squeeze(seg(sn(1),:,sn(3),cur4,cur5)));
            set(handles{5},'XLim',voxspace(1)*[1 size(seg,2)],'YLim',[ segRange(1) segRange(2)]);
            set(titleHandles{5},'String',[' Vox AP=', int2str(sn(1)),' FH=',int2str(sn(3)),', ',DimLabels{1},' ',int2str(cur4),DimLabels{2},' ',int2str(cur5)]);

         %axes(handles{6});
%             kid=get(gca,'Children');
%             set(kid(1),'YData',squeeze(seg(sn(1),sn(2),:,cur4,cur5)));
%             M=strrep(strrep('tp #, dir $','#',int2str(cur4)),'$',int2str(cur5));
%             legend(M,'Location','best')
%             title([' Vox RL=', int2str(sn(2)),' AP=',int2str(sn(1))]);        
%             axis([1 size(seg,3) segRange(1) segRange(2)]);

            kid=get(handles{6},'Children');
            set(kid(1),'YData',squeeze(seg(sn(1),sn(2),:,cur4,cur5)));
            set(handles{6},'XLim',voxspace(3)*[1 size(seg,3)],'YLim',[ segRange(1) segRange(2)]);
            set(titleHandles{6},'String',[' Vox RL=', int2str(sn(2)),' AP=',int2str(sn(1)),', ',DimLabels{1},' ',int2str(cur4),DimLabels{2},' ',int2str(cur5)]);

       % axes(handles{7});
%             kid=get(gca,'Children');
%             set(kid(1),'YData',squeeze(seg(:,sn(2),sn(3),cur4,cur5)));
%             M=strrep(strrep('tp #, dir $','#',int2str(cur4)),'$',int2str(cur5));
%             legend(M,'Location','best')
%             title([' Vox RL=', int2str(sn(2)),' FH=',int2str(sn(3))]);        
%             axis([1 size(seg,1) segRange(1) segRange(2)]);
% 
            kid=get(handles{7},'Children');
            set(kid(1),'YData',squeeze(seg(:, sn(2),sn(3),cur4,cur5)));
            set(handles{7},'XLim',voxspace(2)*[1 size(seg,1)],'YLim',[ segRange(1) segRange(2)]);
            set(titleHandles{7},'String',[' Vox RL=', int2str(sn(2)),' FH=',int2str(sn(3)),', ',DimLabels{1},' ',int2str(cur4),DimLabels{2},' ',int2str(cur5)]);

    end

    function defineCH3(varargin)
       % Change the slice for axial view      
        newval = str2num(get(CH3val, 'String'));
        newval = round(newval);
        if newval < 1, 
            newval=1;
        elseif newval > size(seg,3),
            newval=size(seg,3)
        end
        sn(3)=newval;
        set(CH3val, 'String', int2str(sn(3)));
        ReplotAll(varargin)
        whichView=1;
        updateSliceViz(newval);
    end
      function defineCH1(varargin)
       % Change the slice for coronal view    
        newval = str2num(get(CH1val, 'String'));
        newval = round(newval);
        if newval < 1, 
            newval=1;
        elseif newval > size(seg,1),
            newval=size(seg,1)
        end
        sn(1)=newval;
        set(CH1val, 'String', int2str(sn(1)));
        ReplotAll(varargin)
        whichView=2;
        updateSliceViz(newval);

      end
    function defineCH2(varargin)
       % Change the slice for sagittal view   
        newval = str2num(get(CH2val, 'String'));
        newval = round(newval);
        if newval < 1, 
            newval=1;
        elseif newval > size(seg,2),
            newval=size(seg,2)
        end
        sn(2)=newval;
        set(CH2val, 'String', int2str(sn(2)));
        ReplotAll(varargin)
        whichView=3;
        updateSliceViz(newval);
    end      
 
end
