function ret = smooth3nan(data, filt, sz, arg)
%SMOOTH3nan  Smooth 3D data - modified to work with NaN data
%   W = SMOOTH3nan(V) smoothes input data V. The smoothed data is returned
%       in W.
%   
%   W = SMOOTH3nan(V, METHOD) METHOD can be either of the filters 'gaussian'
%       or 'box' (default) and determines the convolution kernel.
%   
%   W = SMOOTH3nan(V, METHOD, SIZE) sets the size of the convolution kernel
%       (default is [3 3 3]). If SIZE is a scalar, the size is interpreted
%       as [SIZE SIZE SIZE].
%   
%   W = SMOOTH3nan(V, METHOD, SIZE, ARG) sets an attribute of the
...
%   ====RJO 07/09/13 ============added nan commands
...
ret=nanconvn(padreplicate(data,padSize),smooth,'nanout','edge');

%framesz=(size(smooth)-1)/2;
framesz =([size(smooth,1) size(smooth,2) size(smooth,3)]-1)/2;
retsz=size(ret);
use1=(1+framesz(1)):(retsz(1)-framesz(1));
use2=(1+framesz(2)):(retsz(2)-framesz(2));
use3=(1+framesz(3)):(retsz(3)-framesz(3));
ret=ret(use1,use2,use3);
...
h = h/nansum(h(:)); %changed to nansum
