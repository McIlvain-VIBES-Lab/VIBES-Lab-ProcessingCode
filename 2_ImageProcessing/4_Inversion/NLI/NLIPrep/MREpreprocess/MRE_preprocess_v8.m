function MRE_preprocess_v8(default)
...
%   
disp('MRE-Zone v8 Data Converter')
...
% ..
% Slice_end
fid=fopen([inpath regoutf],'w');
fprintf(fid,'Array Dimensions \n'); 
fprintf(fid,'%7i %7i %7i \n',mridim);
fprintf(fid,'x coordinates \n');
fclose(fid);
dlmwrite([inpath regoutf],xin,'-append','delimiter',' '); 
fid=fopen([inpath regoutf],'a'); fprintf(fid,'y coordniates \n'); fclose(fid);
dlmwrite([inpath regoutf],yin,'-append','delimiter',' ');
fid=fopen([inpath regoutf],'a'); fprintf(fid,'z coordniates \n'); fclose(fid);
dlmwrite([inpath regoutf],zin,'-append','delimiter',' ');
for ii=1:mridim(3)
    dlmwrite([inpath regoutf],regs(:,:,ii),'-append','delimiter',' ');
end

% % Tet mesh assumption (Switch x and y directions)
% regoutf_tet=[outstm 'tet.regstack'];
% % Format:
% % StackSize
% % x coords
% % y coords
...
% % ..
% % Slice_end
% fid=fopen([inpath regoutf_tet],'w');
% fprintf(fid,'Array Dimensions \n'); 
% fprintf(fid,'%7i %7i %7i \n',mridim);
% fprintf(fid,'x coordinates \n');
% fclose(fid);
% dlmwrite([inpath regoutf_tet],yin,'-append','delimiter',' '); 
% fid=fopen([inpath regoutf_tet],'a'); fprintf(fid,'y coordniates \n'); fclose(fid);
% dlmwrite([inpath regoutf_tet],xin,'-append','delimiter',' ');
% fid=fopen([inpath regoutf_tet],'a'); fprintf(fid,'z coordniates \n'); fclose(fid);
% dlmwrite([inpath regoutf_tet],zin,'-append','delimiter',' ');
% for ii=1:mridim(3) % Transpose each slice to fit tet mesh assumption
%     dlmwrite([inpath regoutf_tet],regs(:,:,ii)','-append','delimiter',' ');
% end
...
if(strcmpi(meshtype,'hex')) % Output Hexahedral runfiles
    output_runfiles_hex(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg);
elseif(strcmpi(meshtype,'tet'))
    output_runfiles_tet(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,pbcf2,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg);
end
