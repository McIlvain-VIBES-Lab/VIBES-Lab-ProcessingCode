function [OSS_SNR,Motion_SNR,OSS_SNR_Dist,Motion_SNR_Dist,oss,ons,varargout]=Strain_SNR_and_NITI_from_phase(P,mask,res,filtwidth,ord,Nfit,DTI,niti_prop)
...
%
%% Udpate 10 April 2020, Elijah Van Houten:
% Uses DTI and NITI_prop data to calculate the sensitivity to the NITI
% material property values. This is done by calculating the stress derivatives
% d\sigma/d\theta, where \theta are the NITI parameters \mu, \zeta and \phi
% respectively. The NITI_sens value is then calculated as
% norm(d\sigma(\theta))/norm(\sigma(\theta)).

% Matts function call:
%[OSS_SNR,Motion_SNR,OSS_SNR_Dist,Motion_SNR_Dist,oss,ons,varargout]=Strain_SNR_and_NITI_from_phase(squeeze(Ur(:,:,:,:,1)+1i*Ui(:,:,:,:,1)),mask,[2.05 2.05 2.5]/1000,0.5,2,3,V1,niti_prop)
...
niti_calc=false; %By default do not calculate NITI Sensitivity
prop_def=[3300 0.5 0.5]; %Default property valued for NITI Sensitivity Analysis
...
s=size(P);
if(ndims(P)==4)&&(s(4)==3)
    %Displacement Data Passed in P Variable
    Nph=8;
    Ur=real(P);
    Ui=imag(P);
    clear P;
    Ph=0:2*pi/Nph:2*pi*(Nph-1)/Nph;
    P=zeros(s(1),s(2),s(3),Nph,3);
    for ii=1:Nph
        P(:,:,:,ii,1)=real((Ur(:,:,:,1)+1i*Ui(:,:,:,1))*exp(1i*Ph(ii)));
        P(:,:,:,ii,2)=real((Ur(:,:,:,2)+1i*Ui(:,:,:,2))*exp(1i*Ph(ii)));
        P(:,:,:,ii,3)=real((Ur(:,:,:,3)+1i*Ui(:,:,:,3))*exp(1i*Ph(ii)));
    end
    s=size(P);
end
...
if(nargin<7)
    disp('No Fiber Axis data provided, NITI Sensitivity Cannot be Calculated. ')
elseif(~isequal([s(1:3) 3],size(DTI)))
    error('DTI and P do not have consistent dimensions')
else
    niti_calc=true;
    disp('Fiber Axis data provided, NITI Sensitivity Will be Calculated. ')
end
if(nargin<8&&niti_calc)
    niti_prop=zeros(s(1),s(2),s(3),3);
    niti_reg=(vecnorm(DTI,2,4)>0.1);
    for ii=1:3
        niti_prop(:,:,:,ii)=prop_def(ii)*niti_reg;
    end
    disp(['No NITI Properties provided, using default values [\mu,\zeta,\phi] :: ',num2str(prop_def)])
elseif(~isequal([s(1:3) 3],size(niti_prop)))
    error('niti_prop and P do not have consistent dimensions')
end
if(mod(Nfit,2)==0)
...
if ~niti_calc
    ...
    [oss,ons,niti_sens]=OSS_calc(Ur,Ui,mask,res,filtsiz,filtwidth,ord,Nfit,Parr,DTI,niti_prop);
    varargout={niti_sens};
end
%% OSS Noise Estimate
...
save OSS_SNR.mat filtwidth Nfit ord OSS_SNR Motion_SNR OSS_SNR_Dist Motion_SNR_Dist oss ons ossCD

end % End of Strain SNR calculation

function [oss,ons,varargout]=OSS_calc(Ur,Ui,mask,res,filtsiz,filtwidth,ord,Nfit,Parr,DTI,niti_prop)
...
jnk=DTI;
DTI=zeros([dim+2*bufsiz,3]);
for ii=1:3
    [DTI(:,:,:,ii)]=zerobuffer3d(jnk(:,:,:,ii),bufsiz,'add');
end

jnk=niti_prop;
niti_prop=zeros([dim+2*bufsiz,3]);
for ii=1:3
    [niti_prop(:,:,:,ii)]=zerobuffer3d(jnk(:,:,:,ii),bufsiz,'add');
end
...
if ~niti_calc
    ...
    if(Parr)
        parfor ii=1+bufsiz:dim(1)+bufsiz
            ...
            % classifiation, putting the loop body into a seperate function
            % solved this problem.
            [oss(ii,:,:),ons(ii,:,:),curldsp(ii,:,:,:)]=process_slice(ii,Ur,Ui,mask,ord,lf,dim,bufsiz,ntrms,res,DTI,niti_prop);
        end
    else
        for ii=1+bufsiz:dim(1)+bufsiz
            [oss(ii,:,:),ons(ii,:,:),curldsp(ii,:,:,:),niti_sens(ii,:,:,:)]=process_slice(ii,Ur,Ui,mask,ord,lf,dim,bufsiz,ntrms,res,DTI,niti_prop);
        end
    end
...
if niti_calc
    junk=niti_sens;
    niti_sens=zeros([dim 6])+1i*zeros([dim 6]);
    for ii=1:6
        niti_sens(:,:,:,ii)=zerobuffer3d(junk(:,:,:,ii),bufsiz,'remove');
    end
    varargout={niti_sens};
end
...
function [oss_ii,ons_ii,curldsp_ii,varargout]=process_slice(ii,Ur,Ui,mask,ord,lf,dim,bufsiz,ntrms,res,DTI,niti_prop)
...
if nargin>11
    niti_calc=true;
    niti_sens_ii=zeros(dim2(2),dim2(3),6);
end
    ...
    for kk=1+bufsiz:dim(3)+bufsiz
        if mask(ii,jj,kk)==1
            ...
            if ~niti_calc
                ...
                [oss_ii(jj,kk),ons_ii(jj,kk),curldsp_ii(jj,kk,:),niti_sens_ii(jj,kk,:)]=octshearstain_curl(ar,ai,dx,dy,dz,DTI(ii,jj,kk,:),niti_prop(ii,jj,kk,:));
            end
        end
    end
end
varargout={niti_sens_ii};
end

function [ossval,onsval,curlu,varargout]=octshearstain_curl(ar,ai,dx,dy,dz,dtivec,nitiprops)
...
% Perform NITI Sensitivity Analysis
if niti_calc
    strain=[sxx;syy;szz;syz;sxz;sxy];
    [Cprime,CPrimesens]=NITIStiffnessMatrix(nitiprops(1),nitiprops(2),nitiprops(3));
    C=OrthotropicRotation(Cprime,squeeze(dtivec));
    stress=C*strain;
    for ii=1:3
        Csens=OrthotropicRotation(CPrimesens(:,:,ii),squeeze(dtivec));
        stresssens=Csens*strain;
        nitisensloc(ii)=norm(stresssens)/norm(stress);
        nitisensloc(3+ii)=norm(stresssens);
    end
    varargout={nitisensloc};
end
end
...
function C=OrthotropicRotation(Cprime,n)

% Normalize Fiber Axis Vector:
n=n/norm(n);
% Rotation matrix
a=[n(1) -n(2)/sqrt(n(1)^2+n(2)^2) -n(1)*n(3)/sqrt(n(1)^2+n(2)^2);
    n(2) n(1)/sqrt(n(1)^2+n(2)^2) -n(2)*n(3)/sqrt(n(1)^2+n(2)^2);
    n(3)  0 sqrt(n(1)^2+n(2)^2)];

% Bond transofration (Eq 6.23)

%So i can copy and paste from zeinceiwcz
t11=a(1,1);t12=a(1,2);t13=a(1,3);
t21=a(2,1);t22=a(2,2);t23=a(2,3);
t31=a(3,1);t32=a(3,2);t33=a(3,3);

Tsig=[t11*t11 t12*t12 t13*t13 2*t11*t12 2*t12*t13 2*t13*t11;
    t21*t21 t22*t22 t23*t23 2*t21*t22 2*t22*t23 2*t23*t21;
    t31*t31 t32*t32 t33*t33 2*t31*t32 2*t32*t33 2*t33*t31;
    t11*t21 t12*t22 t13*t23 (t11*t22 + t12*t21) (t12*t23 + t13*t22) (t13*t21 + t11*t23);
    t21*t31 t22*t32 t23*t33 (t21*t32 + t22*t31) (t22*t33 + t23*t32) (t23*t31 + t21*t33);
    t31*t11 t32*t12 t33*t13 (t31*t12 + t32*t11) (t32*t13 + t33*t12) (t33*t11 + t31*t13)];

Teps=[t11*t11 t12*t12 t13*t13 t11*t12 t12*t13 t13*t11;
    t21*t21 t22*t22 t23*t23 t21*t22 t22*t23 t23*t21;
    t31*t31 t32*t32 t33*t33 t31*t32 t32*t33 t33*t31;
    2*t11*t21 2*t12*t22 2*t13*t23 (t11*t22 + t12*t21) (t12*t23 + t13*t22) (t13*t21 + t11*t23);
    2*t21*t31 2*t22*t32 2*t23*t33 (t21*t32 + t22*t31) (t22*t33 + t23*t32) (t23*t31 + t21*t33);
    2*t31*t11 2*t32*t12 2*t33*t13 (t31*t12 + t32*t11) (t32*t13 + t33*t12) (t33*t11 + t31*t13)];
%Eq 6.38
C=Teps'*Cprime*Teps;
...
function [S,Ssens]=NITIStiffnessMatrix(mu,zeta,phi)
S=zeros(6,6);
S(1,1)=(4/3)*mu*(1+(4/3)*zeta);
S(2,2)=(4/3)*mu*(1+(1/3)*zeta);
S(3,3)=(4/3)*mu*(1+(1/3)*zeta);
S(1,2)=-(2/3)*mu*(1+(4/3)*zeta);
S(2,1)=-(2/3)*mu*(1+(4/3)*zeta);
S(1,3)=-(2/3)*mu*(1+(4/3)*zeta);
S(3,1)=-(2/3)*mu*(1+(4/3)*zeta);
S(2,3)=-(2/3)*mu*(1-(2/3)*zeta);
S(3,2)=-(2/3)*mu*(1-(2/3)*zeta);
S(4,4)=mu*(1+phi);
S(5,5)=mu;
S(6,6)=mu*(1+phi);
...
Ssens=zeros(6,6,3);
%dSdmu
Ssens(1,1)=(4/3)*(1+(4/3)*zeta);
Ssens(2,2)=(4/3)*(1+(1/3)*zeta);
Ssens(3,3)=(4/3)*(1+(1/3)*zeta);
Ssens(1,2)=-(2/3)*(1+(4/3)*zeta);
Ssens(2,1)=-(2/3)*(1+(4/3)*zeta);
Ssens(1,3)=-(2/3)*(1+(4/3)*zeta);
Ssens(3,1)=-(2/3)*(1+(4/3)*zeta);
Ssens(2,3)=-(2/3)*(1-(2/3)*zeta);
Ssens(3,2)=-(2/3)*(1-(2/3)*zeta);
Ssens(4,4)=(1+phi);
Ssens(5,5)=1;
Ssens(6,6)=(1+phi);

%dSdzeta
Ssens(1,1,2)=(4/3)*mu*(4/3);
Ssens(2,2,2)=(4/3)*mu*(1/3);
Ssens(3,3,2)=(4/3)*mu*(1/3);
Ssens(1,2,2)=-(2/3)*mu*(4/3);
Ssens(2,1,2)=-(2/3)*mu*(4/3);
Ssens(1,3,2)=-(2/3)*mu*(4/3);
Ssens(3,1,2)=-(2/3)*mu*(4/3);
Ssens(2,3,2)=(2/3)*mu*(2/3);
Ssens(3,2,2)=(2/3)*mu*(2/3);
