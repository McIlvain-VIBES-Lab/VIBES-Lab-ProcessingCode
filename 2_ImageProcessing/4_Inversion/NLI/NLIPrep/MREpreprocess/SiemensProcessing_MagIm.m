...
PFolder(1)=input('Select First Maginitude Directory: >>');
PFolder(2)=input('Select Second Magnitude Directory: >>');
PFolder(3)=input('Select Third Magnitude Directory: >>');
...
if isequal(DicomInfo(1).Private_0051_100e,DicomInfo(2).Private_0051_100e,DicomInfo(3).Private_0051_100e)==0
    error('These directories do not have the same slice direction');
else 
...
if isequal(DicomInfo(1).PixelSpacing,DicomInfo(2).PixelSpacing,DicomInfo(3).PixelSpacing)==0 && ...
    ...
    error('These directories do not have the same voxel size');
else 
...
if isequal(DicomInfo(1).Rows,DicomInfo(2).Rows,DicomInfo(3).Rows)==0 && ...
    ...
    error('These directories do not have the same # of Rows and/or Columns');
else 
...
%     error('These directories do not have the same Rescale Slope/Intercept');
% else 
...
if isequal(DicomInfo(1).CardiacNumberOfImages,DicomInfo(2).CardiacNumberOfImages,DicomInfo(3).CardiacNumberOfImages)==0
    error('These directories do not have the number of cardiac cycles');
else 
...
RawMag=zeros(DicomInfo(1).Rows,DicomInfo(1).Columns,nslices,ndir,DicomInfo(1).CardiacNumberOfImages); 
...
for jj=1:3
    ...
    for kk=1:length(dicom_images);
        ...
        RawMag(:,:,slice,jj,phase)=dicomread(dicom_images(kk).name); 
    end
    cd ../
end
...
MagIm=mean(mean(RawMag,5),4); 

save RawMag.mat RawMag
%Ur=A.*cos(P); 
%Ui=A.*sin(P); 
montagestack(MagIm);title('Magnitude Image');colorbar; 
...
Motion2Image=M2I;
save MagIm.mat MagIm voxsize_mm RHcoord res