function MRE_preprocess_v9p3_mcilvain(default)
...
if ~noDTI
    ...
    writeregstackfile(V1(:,:,:,2),size(V1(:,:,:,2)),xin,yin,zin,DTIstackfx); % 2nd index is AP, which is (:,1,1), which is our Y
...
% Uopdate 12/9/2019 to modlularize these subroutines



if(strcmpi(meshtype,'hex')) % Output Hexahedral runfiles
    [filelist]=output_runfiles_hex_v9_isovisc_mcilvain(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg,noDTI);
 %   output_submitdiscovery_hex_v9_isovisc(inpath,noreg,noDTI,filelist);
    
    % Havent automated the NITI submit files yet
    %output_runfiles_hex_v9_NITI(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg,noDTI);
    %output_submitdiscovery_hex_v9_NITI(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg,noDTI);
    
elseif(strcmpi(meshtype,'tet'))
    output_runfiles_tet_v9(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,pbcf2,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg,noDTI);
end
