...
if(masktyp==1)
    ...
    for ii=1:size(mask,3)
        ...
        mask(:,:,ii)=imerode(mask(:,:,ii),ones(5,5));
    end
else
    load('Mask_left.mat')
end
mask=double(mask);
montagestack(mask);title('Mask')
...
maskprelude=mask;
maskprelude(size(mask,1)/2:size(mask,1)/2+3,:,size(mask,3)/2)=1; % Join feet

mask_rep = repmat(maskprelude,[1 1 1 nk]);
...
mreParams.subj = 'mre_for_inversion2';
