...
% Use different mudef for each frequency
freqvals=[10 15 20 25 30 35 40 45 50];
mur_freq=[1000 1200 1400 1500 1700 2000 2500 3000 3300];
mui_freq=[400 500 600 700 800 900 1000 1100 1188];
[Y,Ifreq]=min(abs(freqHz-freqvals));

DR=1/2*mui_freq(Ifreq)/mur_freq(Ifreq); % Default damping ratio
...
elseif(meshstrat==2)||(meshstrat==3)  %  Process geometry and interpolate to get desired nodes per wavelength if meshstrat = 2.
    ...
    if(par)
        ...
        if(noreg)
            ...
            [Yq,Xq,Zq]=meshgrid(yout,xout,zout);
            regs_int = interp3(yin,xin,zin,regs,Yq,Xq,Zq,'nearest');
            clear Yq Xq Zq
        end
    end
...
sprsf_file=[outstm '.regstack.sfweight'];
...
viscoutputfv7p3isoincomp=[outstm  '_G' sprintf('%0.0f',muest) '.v7.3.inv.iso.incomp.visc_SPon'];
...
fprintf(fid,'1.d-12,1.d-12,1.d-12 \n');
fprintf(fid,'1.d-12,1.d-12,1.d-12 \n');
...
fprintf(fid,'#PBS -l nodes=6:ppn=8 \n');
fprintf(fid,'# request some hours of wall time  \n');
fprintf(fid,'#PBS -l walltime=70:00:00  \n');
...
fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MREv7p40/MRE-Zone.v7.40.discov ');fprintf(fid,'%c',[viscrfilev7p3]);fprintf(fid,'\n');
fprintf(fid,'exit 0  \n');
fclose(fid);
...
viscoutputfv7p3isoincomp_noSP=[outstm  '_G' sprintf('%0.0f',muest) '.v7.3.inv.iso.incomp.visc_SPoff_SF0p0015_CG2p2'];
...
fprintf(fid,'echo Nodes $nnodes \n');
fprintf(fid,'echo Procs $nprocs \n');
fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MREv7p40/MRE-Zone.v7.40.discov ');fprintf(fid,'%c',[viscrfilev7p3_noSP]);fprintf(fid,'\n');
fprintf(fid,'exit 0  \n');
fclose(fid);
