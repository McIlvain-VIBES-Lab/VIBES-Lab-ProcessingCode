function output_runfiles_hex_v9_liang(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg,noDTI,SubjectName)
%output_runfiles_hex outputs runfiles and submit files for a hexahedral

% MoDIFICATIONS: 
% change num of iterations from 100 to 50 for four times
% change memory and hour limit for four times 
%mesh
...
%% Hexahedral runfiles
  ...
  % Build MRE-submit file
   viscsubnamev9='McIlvain-Submitv9_visc_incomp_SPon';
    ...
    fprintf(fid,'#SBATCH --time=5:00:00 \n');
    ...
    fprintf(fid, '#SBATCH --mail-user=hl3793@columbia.edu \n');
    ...
    fprintf(fid,'RUNFILE="runfile-v9visc.dat_SP1en9" \n');
  ...
  % Build MRE-submit file
   viscsubnamev9='McIlvain-Submitv9_visc_incomp';
       runfile_dir = ['/insomnia001/depts/mcilvain/users/mcilvain/', SubjectName, '/hex/', SubjectName, '_voxelmesh'];
    ...
    fprintf(fid,'SIF_PATH="/insomnia001/depts/mcilvain/projects/NLI/mre-neuro-processing-final.sif" \n');
    fprintf(fid, 'RUNFILE_DIR="%s" \n', runfile_dir);
    fprintf(fid,'RUNFILE="runfile-v9_visc_incomp.dat" \n');
    fprintf(fid, 'cd "$RUNFILE_DIR" \n');
    fprintf(fid, 'apptainer exec --bind "$RUNFILE_DIR":/mnt "$SIF_PATH" bash -c ');
    fprintf(fid, '''export I_MPI_FABRICS=shm:ofi && /opt/intel/oneapi/mpi/bin/mpiexec.hydra -n 30 /MREv9p37a/MRE-Zone.discov /mnt/''"$RUNFILE"\n');
    fprintf(fid,'date \n');
   ...
   viscsubnamev9='McIlvain-Submitv9_visc_incomp_highres';
   runfile_dir = ['/insomnia001/depts/mcilvain/users/mcilvain/', SubjectName, '/hex/', SubjectName, '_voxelmesh'];
   ...
   fid=fopen([inpath viscsubnamev9],'w');
    ...
    fprintf(fid,'#SBATCH --account=mcilvain \n');
    fprintf(fid,'#SBATCH --job-name=mre_processing \n');
    fprintf(fid,'#SBATCH --time=05:00:00 \n');
    fprintf(fid,'#SBATCH --ntasks=30 \n');
    fprintf(fid,'#SBATCH --cpus-per-task=1 \n');
    fprintf(fid,'#SBATCH --mem=60G \n');
    fprintf(fid,'#SBATCH --partition=standard \n');
    ...
    fprintf(fid,'SIF_PATH="/insomnia001/depts/mcilvain/projects/NLI/mre-neuro-processing-final.sif" \n');
    fprintf(fid, 'RUNFILE_DIR="%s" \n', runfile_dir);
    fprintf(fid,'RUNFILE="runfile-v9_visc_incomp_highres.dat" \n');
    fprintf(fid, 'cd "$RUNFILE_DIR" \n');
    fprintf(fid, 'apptainer exec --bind "$RUNFILE_DIR":/mnt "$SIF_PATH" bash -c ''"export I_MPI_FABRICS=shm:ofi && /opt/intel/oneapi/mpi/bin/mpiexec.hydra -n 30 /MREv9p37a/MRE-Zone.discov /mnt/\\$RUNFILE"'' \n');
    fprintf(fid,'date \n');
    fprintf(fid,'## auto post processing \n');
    fprintf(fid,'srchtag=*RE..*0001*prop.01.mtr\n');
    fprintf(fid,'outstm=$(sed ''21!d'' runfile-v9_visc_incomp)\n');
    ...
    viscsubnamev9='McIlvain-Submitv9_visc_comp';
    runfile_dir = ['/insomnia001/depts/mcilvain/users/mcilvain/', SubjectName, '/hex/', SubjectName, '_voxelmesh'];
    fid=fopen([inpath viscsubnamev9],'w');
    %mvfiles(length(mvfiles)+1).name=viscsubnamev7p3_noSP;
    fprintf(fid,'#!/bin/bash -l \n');
    ...
    fprintf(fid,'SIF_PATH="/insomnia001/depts/mcilvain/projects/NLI/mre-neuro-processing-final.sif" \n');
    fprintf(fid, 'RUNFILE_DIR="%s" \n', runfile_dir);
    fprintf(fid,'RUNFILE="runfile-v9_visc_comp.dat" \n');
    fprintf(fid, 'cd "$RUNFILE_DIR" \n');
    fprintf(fid, 'apptainer exec --bind "$RUNFILE_DIR":/mnt "$SIF_PATH" bash -c ');
    fprintf(fid, '''export I_MPI_FABRICS=shm:ofi && /opt/intel/oneapi/mpi/bin/mpiexec.hydra -n 30 /MREv9p37a/MRE-Zone.discov /mnt/''"$RUNFILE"\n');
    fprintf(fid,'date \n');
