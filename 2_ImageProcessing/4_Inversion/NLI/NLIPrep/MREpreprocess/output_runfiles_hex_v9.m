function output_runfiles_hex_v9(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg,noDTI)
...
%% Hexahedral runfiles
    ...
    fprintf(fid,'1 2 2 \n');
        ...
        viscsubnamev9='MPICH2-Submitv9_visc_incomp_SPon';
        ...
        fprintf(fid,'#PBS -N ');fprintf(fid,'%c',viscoutputfv9isoincomp);fprintf(fid,'\n');
        ...
        fprintf(fid,'rfile=');fprintf(fid,'%c',viscrfilev9_SP);fprintf(fid,'\n');
        fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9p21a/MRE-Zone.discov $rfile \n');
        fprintf(fid,'## auto post processing \n');
        fprintf(fid,'srchtag=*RE..*0001*prop.01.mtr\n');
        fprintf(fid,'outstm=$(sed ''21!d'' $rfile)\n');
        fprintf(fid,'~/code/convcode/MREpostprocessv8.x $outstm$srchtag');fprintf(fid,'\n');
        fprintf(fid,'exit 0  \n');
        fclose(fid);
    ...
    % Build MRE-submit file
    viscsubnamev9_noSP='MPICH2-Submitv9_visc_incomp';
    fid=fopen([inpath viscsubnamev9_noSP],'w');
    %mvfiles(length(mvfiles)+1).name=viscsubnamev7p3_noSP;
    fprintf(fid,'#!/bin/bash -l \n');
    fprintf(fid,'# declare a name for this job \n'); 
    fprintf(fid,'#PBS -N ');fprintf(fid,'%c',viscoutputfv9isoincomp_noSP);fprintf(fid,'\n');
    fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
    fprintf(fid,'#PBS -q default  \n');
    fprintf(fid,'# request  node  \n');
    fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
    ...
    fprintf(fid,'echo Procs $nprocs \n');
	fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
    %fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9/MRE-Zone.discov ');fprintf(fid,'%c',viscrfilev9_noSP);fprintf(fid,'\n');
    fprintf(fid,'rfile=');fprintf(fid,'%c',viscrfilev9_noSP);fprintf(fid,'\n');
    fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9p21a/MRE-Zone.discov $rfile \n');
    fprintf(fid,'## auto post processing \n');
    fprintf(fid,'srchtag=*RE..*0001*prop.01.mtr\n');
    fprintf(fid,'outstm=$(sed ''21!d'' $rfile)\n');
    ...
    % Build MRE-submit file
    viscsubnamev9_highres='MPICH2-Submitv9_visc_incomp_highres';
    fid=fopen([inpath viscsubnamev9_highres],'w');
    fprintf(fid,'#!/bin/bash -l \n');
    fprintf(fid,'# declare a name for this job \n'); 
    fprintf(fid,'#PBS -N ');fprintf(fid,'%c',viscoutputfv9isoincomp_highres);fprintf(fid,'\n');
    fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
    fprintf(fid,'#PBS -q default  \n');
    fprintf(fid,'# request  node  \n');
    fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
    ...
    fprintf(fid,'echo Procs $nprocs \n');
	fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
    %fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9/MRE-Zone.discov ');fprintf(fid,'%c',viscrfilev9_highres);fprintf(fid,'\n');
    fprintf(fid,'rfile=');fprintf(fid,'%c',viscrfilev9_highres);fprintf(fid,'\n');
    fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9p21a/MRE-Zone.discov $rfile \n');
    fprintf(fid,'## auto post processing \n');
    fprintf(fid,'srchtag=*RE..*0001*prop.01.mtr\n');
    fprintf(fid,'outstm=$(sed ''21!d'' $rfile)\n');
    ...
    % Build MRE-submit file
    viscsubnamev9comp_noSP='MPICH2-Submitv9_visc_comp';
    fid=fopen([inpath viscsubnamev9comp_noSP],'w');
    %mvfiles(length(mvfiles)+1).name=viscsubnamev7p3_noSP;
    fprintf(fid,'#!/bin/bash -l \n');
    fprintf(fid,'# declare a name for this job \n'); 
    fprintf(fid,'#PBS -N ');fprintf(fid,'%c',viscoutputfv9isocomp_noSP);fprintf(fid,'\n');
    fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
    fprintf(fid,'#PBS -q default  \n');
    fprintf(fid,'# request  node  \n');
    fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
    ...
    fprintf(fid,'echo Procs $nprocs \n');
	fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
    %fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9/MRE-Zone.discov ');fprintf(fid,'%c',viscrfilev9_compnoSP);fprintf(fid,'\n');
    fprintf(fid,'rfile=');fprintf(fid,'%c',viscrfilev9_compnoSP);fprintf(fid,'\n');
    fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9p21a/MRE-Zone.discov $rfile \n');
    fprintf(fid,'## auto post processing \n');
    fprintf(fid,'srchtag=*RE..*0001*prop.01.mtr\n');
    fprintf(fid,'outstm=$(sed ''21!d'' $rfile)\n');
