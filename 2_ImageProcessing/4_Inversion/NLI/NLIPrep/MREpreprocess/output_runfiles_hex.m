function output_runfiles_hex(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg)
...
%% Hexahedral runfiles
    ...
    if(~noreg) % do not output soft prior runfiles without a supplied segmentation.
        ...
        viscoutputfv8isoincomp=[outstm  '_G' sprintf('%0.0f',muest) '.v8.hex.inv.iso.incomp.visc_SPon'];
        viscrfilev8_SP='runfile-v8visc.dat_SP1en9';
        %mvfiles(length(mvfiles)+1).name=viscrfilev7p3;
        fid=fopen([inpath viscrfilev8_SP],'w');
        ...
        fprintf(fid,'%c',outpath);fprintf(fid,'%c',viscoutputfv8isoincomp);fprintf(fid,'\n');
        ...
        viscsubnamev8='MPICH2-Submitv8p1_visc_incomp_SPon';
        fid=fopen([inpath viscsubnamev8],'w');
        %mvfiles(length(mvfiles)+1).name=viscsubnamev7p3;
        fprintf(fid,'#!/bin/bash -l \n');
        fprintf(fid,'# declare a name for this job \n'); 
        fprintf(fid,'#PBS -N ');fprintf(fid,'%c',viscoutputfv8isoincomp);fprintf(fid,'\n');
        ...
        fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv8p1/MRE-Zone.discov ');fprintf(fid,'%c',viscrfilev8_SP);fprintf(fid,'\n');
        fprintf(fid,'exit 0  \n');
        fclose(fid);
    ...
    viscoutputfv8isoincomp_noSP=[outstm  '_G' sprintf('%0.0f',muest) '.v8.hex.inv.iso.incomp.visc_SPoff_SF0p0015_CG2p2'];
    viscrfilev8_noSP='runfile-v8_visc_incomp.dat';
    %mvfiles(length(mvfiles)+1).name=viscrfilev7p3_noSP;
    fid=fopen([inpath viscrfilev8_noSP],'w');
    fprintf(fid,'Subzone Reconstruction Data File \n');
    fprintf(fid,'Problem Type <0=Forward Problem Only> <1+=Inverse Problem> \n');
    fprintf(fid,'1 \n');
    fprintf(fid,'Node File: \n');
    ...
    fprintf(fid,'0 \n');
    fprintf(fid,'Output File Stem: \n');
    fprintf(fid,'%c',outpath);fprintf(fid,'%c',viscoutputfv8isoincomp_noSP);fprintf(fid,'\n');
    fprintf(fid,'Print Detailed Runtime Debugging and Execution Comments <verb> <file>:\n');
    fprintf(fid,'.false.,.false. \n');
    fprintf(fid,'Material Model <1=isotropic> <2=orthotropic> <3=iso compress>  <4=poro>: \n');
    fprintf(fid,'1 \n');
    ...
    % Build MRE-submit file
    viscsubnamev8_noSP='MPICH2-Submitv8p1_visc_incomp';
    fid=fopen([inpath viscsubnamev8_noSP],'w');
    %mvfiles(length(mvfiles)+1).name=viscsubnamev7p3_noSP;
    fprintf(fid,'#!/bin/bash -l \n');
    fprintf(fid,'# declare a name for this job \n'); 
    fprintf(fid,'#PBS -N ');fprintf(fid,'%c',viscoutputfv8isoincomp_noSP);fprintf(fid,'\n');
    fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
    fprintf(fid,'#PBS -q default  \n');
    fprintf(fid,'# request  node  \n');
    fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
    ...
    fprintf(fid,'echo Procs $nprocs \n');
	fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
    fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv8p1/MRE-Zone.discov ');fprintf(fid,'%c',viscrfilev8_noSP);fprintf(fid,'\n');
    fprintf(fid,'exit 0  \n');
    fclose(fid);
    
    %% MREv8 runfile
    % Iso incompressible run file - viscoelastic - No soft Prior
    viscoutputfv8isoincomp_highres=[outstm  '_G' sprintf('%0.0f',muest) '.v8.hex.inv.iso.incomp.visc_SPoff_SF0p0008_CG4p4_TV5en17'];
    viscrfilev8_highres='runfile-v8_visc_incomp_highres.dat';
    %mvfiles(length(mvfiles)+1).name=viscrfilev7p3_noSP;
    fid=fopen([inpath viscrfilev8_highres],'w');
    fprintf(fid,'Subzone Reconstruction Data File \n');
    fprintf(fid,'Problem Type <0=Forward Problem Only> <1+=Inverse Problem> \n');
    fprintf(fid,'1 \n');
    fprintf(fid,'Node File: \n');
    ...
    fprintf(fid,'0 \n');
    fprintf(fid,'Output File Stem: \n');
    fprintf(fid,'%c',outpath);fprintf(fid,'%c',viscoutputfv8isoincomp_highres);fprintf(fid,'\n');
    fprintf(fid,'Print Detailed Runtime Debugging and Execution Comments <verb> <file>:\n');
    fprintf(fid,'.false.,.false. \n');
    fprintf(fid,'Material Model <1=isotropic> <2=orthotropic> <3=iso compress>  <4=poro>: \n');
    fprintf(fid,'1 \n');
    ...
    % Build MRE-submit file
    viscsubnamev8_highres='MPICH2-Submitv8p1_visc_incomp_highres';
    fid=fopen([inpath viscsubnamev8_highres],'w');
    %mvfiles(length(mvfiles)+1).name=viscsubnamev7p3_noSP;
    fprintf(fid,'#!/bin/bash -l \n');
    fprintf(fid,'# declare a name for this job \n'); 
    fprintf(fid,'#PBS -N ');fprintf(fid,'%c',viscoutputfv8isoincomp_highres);fprintf(fid,'\n');
    fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
    fprintf(fid,'#PBS -q default  \n');
    fprintf(fid,'# request  node  \n');
    fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
    ...
    fprintf(fid,'echo Procs $nprocs \n');
	fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
    fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv8p1/MRE-Zone.discov ');fprintf(fid,'%c',viscrfilev8_highres);fprintf(fid,'\n');
    fprintf(fid,'exit 0  \n');
    fclose(fid);
    
    %% MREv8 runfile
    % Iso compressible run file - viscoelastic - No soft Prior
    viscoutputfv8isocomp_noSP=[outstm  '_G' sprintf('%0.0f',muest) '.v8.inv.iso.comp.visc_SPoff_SF0p0015_CG2p2'];
    viscrfilev8_compnoSP='runfile-v8_visc_comp.dat';
    %mvfiles(length(mvfiles)+1).name=viscrfilev7p3_noSP;
    fid=fopen([inpath viscrfilev8_compnoSP],'w');
    fprintf(fid,'Subzone Reconstruction Data File \n');
    fprintf(fid,'Problem Type <0=Forward Problem Only> <1+=Inverse Problem> \n');
    fprintf(fid,'1 \n');
    fprintf(fid,'Node File: \n');
    ...
    fprintf(fid,'0 \n');
    fprintf(fid,'Output File Stem: \n');
    fprintf(fid,'%c',outpath);fprintf(fid,'%c',viscoutputfv8isocomp_noSP);fprintf(fid,'\n');
    fprintf(fid,'Print Detailed Runtime Debugging and Execution Comments <verb> <file>:\n');
    fprintf(fid,'.false.,.false. \n');
    fprintf(fid,'Material Model <1=isotropic> <2=orthotropic> <3=iso compress>  <4=poro>: \n');
    fprintf(fid,'3 \n');
    ...
    % Build MRE-submit file
    viscsubnamev8comp_noSP='MPICH2-Submitv8p1_visc_comp';
    fid=fopen([inpath viscsubnamev8comp_noSP],'w');
    %mvfiles(length(mvfiles)+1).name=viscsubnamev7p3_noSP;
    fprintf(fid,'#!/bin/bash -l \n');
    fprintf(fid,'# declare a name for this job \n'); 
    fprintf(fid,'#PBS -N ');fprintf(fid,'%c',viscoutputfv8isocomp_noSP);fprintf(fid,'\n');
    fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
    fprintf(fid,'#PBS -q default  \n');
    fprintf(fid,'# request  node  \n');
    fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
    ...
    fprintf(fid,'echo Procs $nprocs \n');
	fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
    fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv8p1/MRE-Zone.discov ');fprintf(fid,'%c',viscrfilev8_compnoSP);fprintf(fid,'\n');
    fprintf(fid,'exit 0  \n');
    fclose(fid);
