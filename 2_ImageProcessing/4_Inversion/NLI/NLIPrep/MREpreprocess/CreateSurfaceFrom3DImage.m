function [eb pb] = CreateSurfaceFrom3DImage(stackfn,stackinfofn,tetmeshres,isoval)
...
%
% Modified by Matt McGarry, 18 oct 2017. Added an optional isoval input,
% which sets the level of the contour. If it was set at exactly 1, some
% nodes end up outside the mask due to rounding error etc, and when the
% images are interpolated back to a 3D stack the top and bottom surfaces
% have holes. The isosurface value can also be used to control the
% smoothness of sharp corners. 
...
if ~ismember(0,isovalues)
    ...
    isovalue = setdiff(isovalues,0)*isoval;
...
elseif stack_param.nRow>=70 && stack_param.nCol<200
