function output_runfiles_tet_v9(outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,regoutf,pbcf,freqHz,dspoutf,outpath,znedgelength,znovlp,noreg,noDTI)
...
%% MREv9 runfile
    ...
    fprintf(fid,'%5.0f.d0,%9.5fd0,%9.3fd0,%9.5fd0,%10.5fd0,%10.5fd0,%7.3fd0,%9.5fd0',[muest 0.0001 2*muest*vcomp/(3*(1-2*vcomp)) 0.0001 -7 -24 0.2 0.0001]);fprintf(fid,'\n');
    ...
    porosubnamev9_noSP='MPICH2-Submitv9_poro';
    fid=fopen([inpath porosubnamev9_noSP],'w');
    fprintf(fid,'#!/bin/bash -l \n');
    fprintf(fid,'# declare a name for this job \n'); 
    fprintf(fid,'#PBS -N ');fprintf(fid,'%c',porooutputfv9_noSP);fprintf(fid,'\n');
    ...
    fprintf(fid,'rfile=');fprintf(fid,'%c',pororfilev9_noSP);fprintf(fid,'\n');
    %fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9/MRE-Zone.discov ');fprintf(fid,'%c',pororfilev9_noSP);fprintf(fid,'\n');
    ...
    % Build MRE-submit file
    tetviscsubnamev9_noSP='MPICH2-Submitv9_tetvisc';
    fid=fopen([inpath tetviscsubnamev9_noSP],'w');
    fprintf(fid,'#!/bin/bash -l \n');
    fprintf(fid,'# declare a name for this job \n'); 
    fprintf(fid,'#PBS -N ');fprintf(fid,'%c',tetviscoutputfv9_noSP);fprintf(fid,'\n');
    fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
    fprintf(fid,'#PBS -q default  \n');
    fprintf(fid,'# request  node  \n');
    fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
    ...
    fprintf(fid,'echo Procs $nprocs \n');
	fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
    %fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9/MRE-Zone.discov ');fprintf(fid,'%c',tetviscrfilev9_noSP);fprintf(fid,'\n');
    fprintf(fid,'rfile=');fprintf(fid,'%c',tetviscrfilev9_noSP);fprintf(fid,'\n');
    fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9p21a/MRE-Zone.discov $rfile \n');
    fprintf(fid,'## auto post processing \n');
    fprintf(fid,'srchtag=*RE..*0001*prop.01.mtr\n');
    fprintf(fid,'outstm=$(sed ''21!d'' $rfile)\n');
    ...
    if(~noreg) % do not output soft prior runfiles without a supplied segmentation.
        ...
        fprintf(fid,'1,1,1,1 \n');
        fprintf(fid,'Property Scalars: \n');
        fprintf(fid,'%5.0f.d0,%8.5fd0,%10.3fd0,%10.3fd0,%10.5fd0,%10.5fd0,%7.3fd0,%9.5fd0',[muest 0.0001 2*muest*vcomp/(3*(1-2*vcomp)) 0.0001 -7 -24 0.2 0.0001]);fprintf(fid,'\n');
        fprintf(fid,'Other Poroealstic constants assumed constant <rho, rhof, rhoa> \n');
        fprintf(fid,'1020.d0, 1000.d0, 150.d0 \n');
        fprintf(fid,'Zone displacement boundary condition option: 1=measured data, 2=seperate file (needs disp files on the following lines),3=calculated displacements \n');
        fprintf(fid,'1 \n');
    ...
    fprintf(fid,'2 2 2 2 \n');
        ...
        % Build MRE-submit file
        porosubnamev9_SP='MPICH2-Submitv9_poroSP';
        fid=fopen([inpath porosubnamev9_SP],'w');
        fprintf(fid,'#!/bin/bash -l \n');
        fprintf(fid,'# declare a name for this job \n'); 
        fprintf(fid,'#PBS -N ');fprintf(fid,'%c',porooutputfv9_SP);fprintf(fid,'\n');
        fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
        fprintf(fid,'#PBS -q default  \n');
        fprintf(fid,'# request  node  \n');
        fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
        ...
        fprintf(fid,'echo Procs $nprocs \n');
		fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
        %fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9/MRE-Zone.discov ');fprintf(fid,'%c',pororfilev9_SP);fprintf(fid,'\n');
        fprintf(fid,'rfile=');fprintf(fid,'%c',pororfilev9_SP);fprintf(fid,'\n');
        fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9p21a/MRE-Zone.discov $rfile \n');
        fprintf(fid,'## auto post processing \n');
        fprintf(fid,'srchtag=*RE..*0001*prop.01.mtr\n');
        fprintf(fid,'outstm=$(sed ''21!d'' $rfile)\n');
    ...
    fprintf(fid,'1 2 1 \n');
        ...
        % Build MRE-submit file
        tetviscsubnamev9_SP='MPICH2-Submitv9_tetviscSP';
        fid=fopen([inpath tetviscsubnamev9_SP],'w');
        fprintf(fid,'#!/bin/bash -l \n');
        fprintf(fid,'# declare a name for this job \n'); 
        fprintf(fid,'#PBS -N ');fprintf(fid,'%c',tetviscoutputfv9_SP);fprintf(fid,'\n');
        fprintf(fid,'# request the queue (enter the possible names, if omitted, serial is the default) \n');
        fprintf(fid,'#PBS -q default  \n');
        fprintf(fid,'# request  node  \n');
        fprintf(fid,'#PBS -l nodes=4:ppn=8 \n');
        ...
        fprintf(fid,'echo Procs $nprocs \n');
		fprintf(fid,'cat $PBS_NODEFILE | uniq \n');
        %fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9/MRE-Zone.discov ');fprintf(fid,'%c',tetviscrfilev9_SP);fprintf(fid,'\n');
        fprintf(fid,'rfile=');fprintf(fid,'%c',tetviscrfilev9_SP);fprintf(fid,'\n');
        fprintf(fid,'mpirun -np $nprocs -hostfile $PBS_NODEFILE /ihome/mmcgarry/code/MRE_mumpsv5/MREv9p21a/MRE-Zone.discov $rfile \n');
        fprintf(fid,'## auto post processing \n');
        fprintf(fid,'srchtag=*RE..*0001*prop.01.mtr\n');
        fprintf(fid,'outstm=$(sed ''21!d'' $rfile)\n');
