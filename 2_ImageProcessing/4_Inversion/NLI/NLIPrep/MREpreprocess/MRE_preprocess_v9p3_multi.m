function MRE_preprocess_v9p3_multi(default)
...
%                  interpolated to a resolutionlower than the data.DTI
...
Ndisp=size(Ur,5);
disp(['Number of measured displacement fields: ' int2str(Ndisp)])
...
wl=1/mean(freqHz)*sqrt(muest/rhoest); % Undamped elastic wavelength, could include full VE shear wavelength if neccesary 
...
for idisp=1:Ndisp
    ...
    if(dispscale)
        ...
        Ur(:,:,:,:,idisp)=Ur(:,:,:,:,idisp)./meanA.*dispscalar;
        Ui(:,:,:,:,idisp)=Ui(:,:,:,:,idisp)./meanA.*dispscalar;
    end
end
...
if(strcmpi(meshtype,'hex')) %%%% HEXAHEDRAL MESHING
    ...
    % outside mask.
    Ur(repmat(mask,[1 1 1 3 Ndisp])==0)=nan;
    Ui(repmat(mask,[1 1 1 3 Ndisp])==0)=nan;
    ...
    elseif(meshstrat==2)||(meshstrat==3)  %  Process geometry and interpolate to get desired nodes per wavelength if meshstrat = 2.
        ...
        if(par)
            ...
            for idisp=1:Ndisp
                needsint(:,:,:,(6*(idisp-1)+1):(6*(idisp-1)+3))=Ur(:,:,:,:,idisp);
                needsint(:,:,:,(6*(idisp-1)+4):(6*(idisp-1)+6))=Ui(:,:,:,:,idisp);
            end
            needsint(:,:,:,6*Ndisp+1)=AnatomicalMRI;
%             needsint(:,:,:,6*Ndisp+2)=regs;
            ...
            for idisp=1:Ndisp
                Urint(:,:,:,:,idisp)=intdata(:,:,:,(6*(idisp-1)+1):(6*(idisp-1)+3));
                Uiint(:,:,:,:,idisp)=intdata(:,:,:,(6*(idisp-1)+4):(6*(idisp-1)+6));
            end
            AnatomicalMRI_int=intdata(:,:,:,6*Ndisp+1);
            %regs_int=intdata(:,:,:,8);
            ...
            for idisp=1:Ndisp
                for ii=1:3
                    Urint(:,:,:,ii,idisp)=SplineInterp3D_withnans(xin,yin,zin,Ur(:,:,:,ii,idisp),xout,yout,zout,2,'no');
                end
                for ii=1:3
                    Uiint(:,:,:,ii,idisp)=SplineInterp3D_withnans(xin,yin,zin,Ui(:,:,:,ii,idisp),xout,yout,zout,2,'no');
                end
            end
    ...
    nodnum=1:numel(Urint(:,:,:,1,1));
    nodnum=reshape(nodnum,size(Urint(:,:,:,1,1)));
    ...
    uvwr=zeros(prod(dim),3,Ndisp);
    uvwi=zeros(prod(dim),3,Ndisp);
        ...
        for jj=1:dim(2)
            for kk=1:dim(3)
                if(nodin(ii,jj,kk))
                    ...
                    for idisp=1:Ndisp
                        uvwr(nn,:,idisp)=[Urint(ii,jj,kk,1,idisp) Urint(ii,jj,kk,2,idisp) Urint(ii,jj,kk,3,idisp)];
                        uvwi(nn,:,idisp)=[Uiint(ii,jj,kk,1,idisp) Uiint(ii,jj,kk,2,idisp) Uiint(ii,jj,kk,3,idisp)];
                    end
                    idx(nn,:)=[ii jj kk];
...
elseif(strcmpi(meshtype,'tet'))  %%%% TETRAHEDRAL MESHING
    ...
    % THe eval funtion in CreateSurfaceFrom3DImage doesnt like it when
    % size(mask,1)>size(mask,2) for some reason. It will freeze at the eval
    % stage. Just make the mask square here to workaround it.  
    
    stackfn=logical(mask);
    maxsiz=max(mridim(1:2));
    if(mridim(2)<maxsiz)
        stackfn(maxsiz,maxsiz,mridim(3))=0;
        sz(1)=maxsiz;
        sz(2)=maxsiz;
    else
        sz(1)=mridim(1);
    ...
    uvwr=zeros(length(nod),3,Ndisp);
    uvwi=uvwr;
    for idisp=1:Ndisp
        [uvwr(:,:,idisp),uvwi(:,:,idisp)]=tet_interp_disp(nod,Ur(:,:,:,:,idisp),Ui(:,:,:,:,idisp),vox,mask,AnatomicalMRI);
    end
...
for idisp=1:Ndisp
    uvwr(:,:,idisp)=uvwr(:,:,idisp)*Motion2Image';
    uvwi(:,:,idisp)=uvwi(:,:,idisp)*Motion2Image';  % Nnx3*3x3=Nn*3, Transpose IS required because we store uvw as COLUMN vectors, and transformation matrices are defined for ROW vectors [x' y' z']=T[x y z]
end
...
%disp files
dspoutf=cell(Ndisp,1);
for idisp=1:Ndisp
    dspoutf{idisp}=[outstm,'.disp.',num2str(idisp),'F',sprintf('%.4g',freqHz(idisp)),'.dsp'];%[outstm '.dsp'];
    fid=fopen([inpath dspoutf{idisp}],'w');
    fprintf(fid,'%7i %15.8e %15.8e %15.8e %15.8e %15.8e %15.8e \n',[ind uvwr(:,1,idisp) uvwi(:,1,idisp) uvwr(:,2,idisp) uvwi(:,2,idisp) uvwr(:,3,idisp) uvwi(:,3,idisp)]');
    fclose(fid);
end
...
if ~noDTI
    ...
    if(DTIsim)
        writeregstackfile(V1(:,:,:,1),size(V1(:,:,:,1)),xin,yin,zin,DTIstackfx); % 
        writeregstackfile(V1(:,:,:,2),size(V1(:,:,:,2)),xin,yin,zin,DTIstackfy); % 
        writeregstackfile(V1(:,:,:,3),size(V1(:,:,:,3)),xin,yin,zin,DTIstackfz); % 
        disp('Using Sim DTI direction assumption')
    else
        
        writeregstackfile(V1(:,:,:,1),size(V1(:,:,:,1)),xin,yin,zin,DTIstackfy); % First index is +L, which is (1,:,1), which is our Y !!!ASSUMING PATIENT L, NOT IMAGE L!!!
        writeregstackfile(-V1(:,:,:,2),size(V1(:,:,:,2)),xin,yin,zin,DTIstackfx);  % 2nd index is +A, which is (:,1,1), which is our -X
        writeregstackfile(V1(:,:,:,3),size(V1(:,:,:,3)),xin,yin,zin,DTIstackfz);  % 3rd index is +H, which is (1,1,:), which is our Z
        disp('Using Axial MRI LAH direction assumption for DTI - Other image orientations will be different')
    end
end
%% Output Runfiles and submit files
% Update 6/24/2020 to modlularize these subroutines

% Build a possible file list
rfilestem='runfile_v9p37';
%%  File 1: Hex incomp default SP off
ifile=1;rfile(ifile).flag=false;
rfile(ifile).ID='isoincomp';
if(strcmpi(meshtype,'hex'))
    rfile(ifile).flag=true;
end

%% File 2: Hex incomp default SP on
ifile=2;rfile(ifile).flag=false;
rfile(ifile).ID='isoincomp_SPon';
if(strcmpi(meshtype,'hex')&&(~noreg))
    rfile(ifile).flag=true;
end

%% File 3: Hex comp default SP off
ifile=3;rfile(ifile).flag=false;
rfile(ifile).ID='isocomp';
if(strcmpi(meshtype,'hex'))
    rfile(ifile).flag=true;
end

%% File 4: tet poro no HC SP off
ifile=4;rfile(ifile).flag=false;
rfile(ifile).ID='poro_noHC';
if(strcmpi(meshtype,'tet'))
    rfile(ifile).flag=true;
end

%% File 5: tet poro no HC SP on
ifile=5;rfile(ifile).flag=false;
rfile(ifile).ID='poro_noHC_SPon';
if(strcmpi(meshtype,'tet')&&(~noreg))
    rfile(ifile).flag=true;
end

%% File 6: tet poro recon HC SP off
ifile=6;rfile(ifile).flag=false;
rfile(ifile).ID='poro_reconHC';
if(strcmpi(meshtype,'tet'))
    rfile(ifile).flag=true;
end

%% File 7: tet poro recon HC SP on
ifile=7;rfile(ifile).flag=false;
rfile(ifile).ID='poro_reconHC_SPon';
if(strcmpi(meshtype,'tet')&&(~noreg))
    rfile(ifile).flag=true;
end

%% File 8: tet visc default SP off
ifile=8;rfile(ifile).flag=false;
rfile(ifile).ID='tetvisc';
if(strcmpi(meshtype,'tet'))
    rfile(ifile).flag=true;
end

%% File 9: tet visc default SP on
ifile=9;rfile(ifile).flag=false;
rfile(ifile).ID='tetvisc_SPon';
if(strcmpi(meshtype,'tet')&&(~noreg))
    rfile(ifile).flag=true;
end

%% File 10: Hex NITI aniso damping default SP off
ifile=10;rfile(ifile).flag=false;
rfile(ifile).ID='NITI_anisodamp_SPoff';
if(strcmpi(meshtype,'hex')&&(~noDTI))
    rfile(ifile).flag=true;
end

%% File 11: Hex NITI iso damping default SP off
ifile=11;rfile(ifile).flag=false;
rfile(ifile).ID='NITI_isodamp_SPoff';
if(strcmpi(meshtype,'hex')&&(~noDTI))
    rfile(ifile).flag=true;
end

%% File 12: Hex NITI aniso damping default SP on
ifile=12;rfile(ifile).flag=false;
rfile(ifile).ID='NITI_anisodamp_SPon';
if(strcmpi(meshtype,'hex')&&(~noDTI)&&(~noreg))
    rfile(ifile).flag=true;
end

%% File 13: Hex NITI iso damping default SP on
ifile=13;rfile(ifile).flag=false;
rfile(ifile).ID='NITI_isodamp_SPon';
if(strcmpi(meshtype,'hex')&&(~noDTI)&&(~noreg))
    rfile(ifile).flag=true;
end
...
nfile=ifile;
modelsiz=[range(nod(:,1)) range(nod(:,2)) range(nod(:,3))];
for ifile=1:nfile
    if(rfile(ifile).flag)        
        [globitr]=output_runfile_v9p37_general(rfilestem,rfile(ifile),ifile,outstm,vox,muest,rhoest,DR,inpath,nodhmgf,elmhmgf,bcoutf,pbcf,regoutf,freqHz,dspoutf,outpath,znedgelength,znovlp);
        % Cluster submit files: Comment the ones out you dont want
        output_submitfile_v9p37_discovery(rfilestem,rfile(ifile),ifile,outstm,inpath,outpath,freqHz,vox,modelsiz,globitr); % Dartmouth
        output_submitfile_v9p37_Caviness(rfilestem,rfile(ifile),ifile,outstm,inpath,outpath,freqHz,vox,modelsiz,globitr); % Delaware
    end
end
...
if(stackdim(1)~=size(stack,1))
    warning(['writeregstackfile: stackdim(1)=' int2str(stackdim(1)) ', size(stack,1)=' int2str(size(stack,1))])
end
if(stackdim(2)~=size(stack,2))
    warning(['writeregstackfile: stackdim(2)=' int2str(stackdim(2)) ', size(stack,2)=' int2str(size(stack,2))])
end
if(stackdim(3)~=size(stack,3))
    warning(['writeregstackfile: stackdim(3)=' int2str(stackdim(3)) ', size(stack,3)=' int2str(size(stack,3))])
end
if(length(x1)~=size(stack,1))
    warning(['writeregstackfile: length(x1)=' int2str(length(x1)) ', size(stack,1)=' int2str(size(stack,1))])
end
if(length(x2)~=size(stack,2))
    warning(['writeregstackfile: length(x2)=' int2str(length(x2)) ', size(stack,2)=' int2str(size(stack,2))])
end
if(length(x3)~=size(stack,3))
    warning(['writeregstackfile: length(x3)=' int2str(length(x3)) ', size(stack,3)=' int2str(size(stack,3))])
end
