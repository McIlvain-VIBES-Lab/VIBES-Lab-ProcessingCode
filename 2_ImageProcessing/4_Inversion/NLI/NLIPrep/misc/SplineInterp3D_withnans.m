function [ vout ] = SplineInterp3D_withnans(xin,yin,zin,vin,xout,yout,zout,btyp,contspline,nancheck)
...
%    nancheck(optional): Set to true to set interpolated values to nan if
%          they are closer to nan values than real values in the input stack
%   OUTPUT
...
for ii=1:dim(1)
    for jj=1:dim(2)
        VoutZ(ii,jj,:)= SplineInterp1D_withnans(zin,vin(ii,jj,:),zout,btyp,contspline);
        if(nancheck)
...
for ii=1:dim(1)
    for jj=1:dimout(3)
        VoutYZ(ii,:,jj)=SplineInterp1D_withnans(yin,VoutZ(ii,:,jj),yout,btyp,contspline);
        if(nancheck)
...
for ii=1:dimout(2)
    for jj=1:dimout(3)
        vout(:,ii,jj)=SplineInterp1D_withnans(xin,VoutYZ(:,ii,jj),xout,btyp,contspline);
        if(nancheck)
