...
function MRE_v9_process_folder(fold)
...
f=dir;
if(length(f)>3)
    disp('Multiple meshes in folder')
    for jj=3:length(f)
        if(f(jj).isdir)
            disp(f(jj).name)
        end        
...
for ii=1:length(f0001)
    ...
    % Hard coded for mf.mtr files: eval(['!~/Discovery_home/code/MREpostprocess/MREpostprocessv9MF.x ' f0001(ii).name])
      eval(['!~/Discovery_home/code/convcode_old/MREpostprocessv9.x ' f0001(ii).name])     % Update this path with the path to this compiled code on your system
end

fconv=dir(['*.prop.01.RE.convinfo']);
for ii=1:length(fconv)
    Plotpostprocess7p3(fconv(ii).name,'fig')  % THis code works for all versions, dont need to update. 
end
...
for ii=1:length(fnod)
    ...
    for jj=1:length(fmtr)
        disp(['Mtr file ' fmtr(jj).name])
        MRE_plotv9_mask_Linear_Function(fnod(ii).name,fmtr(jj).name)
    end
end

% This line copies the results to a convinient location to get them when
% you are running large batches of inversions
