...
program MREpostprocessv7p3

  implicit none
  
  
  character*500 fname,mtrf,stm
  character*2 mstr
  integer ii,jj,kk,nset,maxitr,ind,ijunk,N,imtr
  logical fexist
  ...
  print *,'maxitr = ',maxitr
  allocate(meanv(3,2,maxitr),stdv(3,2,maxitr),medv(3,2,maxitr))
  allocate(maxv(3,2,maxitr),minv(3,2,maxitr),lowpcnt(3,2,maxitr),highpcnt(3,2,maxitr))
  meanv(:,:,:)=0.0
  medv(:,:,:)=0.0
  stdv(:,:,:)=0.0
  maxv(:,:,:)=0.0
  ...
  open(unit=9,file=fname,status='old')
    read(9,*) meshind(1,1),meshind(1,2),meshind(1,3)
    read(9,*) meshind(2,1),meshind(2,2),meshind(2,3)
  close(9)
  ...
  do imtr=1,3
	...
	if(ii.lt.maxitr) call system('rm '//trim(mtrf))
  ...
  print *,'minv(1,1,:) ',minv(1,1,1),minv(2,1,1),minv(3,1,1)
  ...
  do imtr=1,3
    ...
    open(unit=9,file=trim(stm) // '.prop.' // mstr // '.RE.convinfo',status='unknown')
      print *,maxitr,imtr
      do ii=1,maxitr
        write(9,*) ii,meanv(imtr,1,ii),stdv(imtr,1,ii),minv(imtr,1,ii),lowpcnt(imtr,1,ii)&
    ...
    open(unit=9,file=trim(stm) // '.prop.' // mstr // '.IM.convinfo',status='unknown')
      print *,maxitr,imtr
      do ii=1,maxitr
        write(9,*) ii,meanv(imtr,2,ii),stdv(imtr,2,ii),minv(imtr,2,ii),lowpcnt(imtr,2,ii)&
...
end program MREpostprocessv7p3  
