...
eval(sprintf('!cp LiptonDistortionInsomnia/%s/outFieldMapNiiFile.nii %s/File_Storage/QSM/outFieldMapNiiFile.nii',dirlist(ii).name,dirlist(ii).name))
end
%%
dirlist = dir('2023*')
...
cd ../../
!$FSLDIR/bin/flirt -in File_Storage/QSM/FMmag.nii -ref t2stack.nii -out File_Storage/QSM/FMmag2MRE.nii -omat File_Storage/QSM/FMmag2MRE.mat -dof 6
!$FSLDIR/bin/flirt -in File_Storage/QSM/FM.nii -ref t2stack.nii -out File_Storage/QSM/FM2MRE.nii -init File_Storage/QSM/FMmag2MRE.mat -applyxfm
cd File_Storage
mkdir('DistortionCorrectionFiles')
!mv QSM/ DistortionCorrectionFiles/
cd DistortionCorrectionFiles/QSM
!gunzip -f *.nii.gz
cd ../../../../
