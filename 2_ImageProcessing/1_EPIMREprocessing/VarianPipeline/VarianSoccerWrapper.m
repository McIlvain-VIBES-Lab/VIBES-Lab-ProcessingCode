...
code_path = '/Volumes/McIlvainDrive2/VIBES-Lab-ProcessingCode/2_ImageProcessing/1_EPIMREprocessing/VarianPipeline';
...
subjpath = pwd; 
[info,SubjectName] = VarianSetup(subjpath,code_path);
...
%% Anatomical and FLAIR Scan DCM to NII
items = dir('SAGITTAL_T1_VOLUME');
for k = 1:length(items)
    itemName = items(k).name;
    fullPath = fullfile('SAGITTAL_T1_VOLUME', itemName);
    if items(k).isdir && ~strcmp(itemName, '.') && ~strcmp(itemName, '..')
        rmdir(fullPath, 's'); 
    end
end
!/Applications/MRIcron/dcm2niix -z n -b n SAGITTAL_T1_VOLUME
!mv SAGITTAL_T1_VOLUME/*.nii SAGITTAL_T1_VOLUME.nii
items = dir('3D_Sag_T2_FLAIR_Cube');
for k = 1:length(items)
    itemName = items(k).name;
    fullPath = fullfile('3D_Sag_T2_FLAIR_Cube', itemName);
    if items(k).isdir && ~strcmp(itemName, '.') && ~strcmp(itemName, '..')
        rmdir(fullPath, 's'); 
    end
end
!/Applications/MRIcron/dcm2niix -z n -b n 3D_Sag_T2_FLAIR_Cube
!mv 3D_Sag_T2_FLAIR_Cube/*.nii 3D_Sag_T2_FLAIR_Cube.nii
...
%% Section 3
% clean up data
VarianCleanData(mreParams,mask,Zmotion,Ymotion,Xmotion,t2stack,OSS_SNR)

%% Section 4
%disp("Running Freesurfer on Insomnia")
system(['sh /Volumes/McIlvainDrive2/VIBES-Lab-ProcessingCode/2_ImageProcessing/1_EPIMREprocessing/VarianPipeline/freesurfer_insomnia.sh']);
