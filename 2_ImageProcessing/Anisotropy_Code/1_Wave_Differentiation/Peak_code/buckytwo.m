function [V1x,V2x,A1x,A2x,V1index,V2index] = buckytwo(Bifo_all,Brfo_all,mask)
...
V1x = zeros(y,x,z,3); V2x = zeros(y,x,z,3);
A1x = zeros(y,x,z); A2x = zeros(y,x,z);
...
for kk = 1:z
    for jj = 1:x
        for ii = 1:y
            if mask(ii,jj,kk)
                [V1, V2, A1, A2] = bucky_twopeaks_v2(allnvec,col(UT_mask(ii,jj,kk,:)));
...
allnvec_mat = permute(repmat(allnvec,[1 1 y x z]),[3 4 5 2 1]);

V1index = zeros(y,x,z);
V2index = zeros(y,x,z);
dot_buckyV1 = zeros(y,x,z,length(allnvec(:,1)));
dot_buckyV2 = zeros(y,x,z,length(allnvec(:,1)));
tic
...
toc
 ...
 save('BuckyOut.mat','V1x','V2x','A1x','A2x','V1index','V2index')