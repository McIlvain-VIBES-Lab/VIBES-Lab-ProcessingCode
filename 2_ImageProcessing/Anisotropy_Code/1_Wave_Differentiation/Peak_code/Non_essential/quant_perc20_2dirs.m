%% OUTPUTS
% Uslowall - concatenated slow waves (0,1,2 >20%) between AP & LR
% Ufastall - concatenated fast waves (0,1,2 >20%) between AP & LR
% Uall - percentage of 


%% INPUTS

cd /Volumes/CLJ-001/Project_Data/HOCKEY/1_PRESEASON/CLJ_HOCKEY_STUDY/1709/Buckley_Tbi_Brain/MRE/AP
load Buckyout.mat
load slowfasttwo.mat
load hockey_pre_AP_brain_1709.mat
[percent20_AP,Unorms_AP,perc20_AP] = percent(A1x,A2x,U_s_V1,U_f_V1,U_s_V2,U_f_V2,mask);
U_s = U_s_V1+U_s_V2;
totalvox = sum(sum(sum(abs(U_s)>0)));
V1_AP = V1x; V2_AP = V2x;

cd /Volumes/CLJ-001/Project_Data/HOCKEY/1_PRESEASON/CLJ_HOCKEY_STUDY/1709/Buckley_Tbi_Brain/MRE/LR
load Buckyout.mat
load slowfasttwo.mat
load hockey_pre_LR_brain_1709.mat
...
%USIall = Us1perc20_SI+Uf1perc20_SI+Us2perc20_SI+Uf2perc20_SI; 
...
%USIallperc = sum(sum(sum(USIall==4)))/totalvox;
...
V1s_AP = zeros(size(mask)); V1f_AP = zeros(size(mask)); V2s_AP = zeros(size(mask)); V2f_AP = zeros(size(mask));
V1s_LR = zeros(size(mask)); V1f_LR = zeros(size(mask)); V2s_LR = zeros(size(mask)); V2f_LR = zeros(size(mask));
...
save('SlowFastSummation.mat','Uall','TW_cps','Vslow','Vfast','Uslowall','Ufastall','Vmult')
