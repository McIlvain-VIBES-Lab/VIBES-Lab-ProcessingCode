function [Ui,U_s_V1,U_f_V1,U_s_V2,U_f_V2]=slow_fast_twopeaks_exp3(Brfo_all,Bifo_all,mask,dtiall,V1index,V2index,A1x,A2x)
...
% m_f = m_sxA
 ...
 for nn = 1:length(allnvec(:,1))
     m_si(:,:,:,:,nn) = cross(squeeze(allnvec_mat(:,:,:,:,nn)),A,4); 
     ...
     m_fi(:,:,:,:,nn) = cross(squeeze(allnvec_mat(:,:,:,:,nn)),squeeze(m_si_unit(:,:,:,:,nn)),4);
    ...
    for ii = 1:xy
        for jj = 1:xy
            for kk = 1:z
                ...
                U_s_doti(ii,jj,kk,nn) = squeeze(dot(Ui(ii,jj,kk,:,nn),squeeze(m_si_unit(ii,jj,kk,:,nn)),4));
                U_f_doti(ii,jj,kk,nn) = squeeze(dot(Ui(ii,jj,kk,:,nn),squeeze(m_fi_unit(ii,jj,kk,:,nn)),4));
            end
        end
    end
end
