function [Ui,U_s_V1,U_f_V1,U_s_V2,U_f_V2]=slow_fast_twopeaks_exp2(Brfo_all,Bifo_all,mask,dtiall,V1index,V2index,A1x,A2x)
...
for nn = 1:length(allnvec(:,1))
    ...
    for jj = 1:xy
        for kk = 1:z
            m_si(:,jj,kk,:,nn) = reshape(cross(squeeze(allnvec_mat(:,jj,kk,:,nn)),squeeze(A(:,jj,kk,:)),2),[xy 1 1 3]);
            m_sx(:,jj,kk,nn) = squeeze(m_si(:,jj,kk,1,nn)); m_sy(:,jj,kk,nn) = squeeze(m_si(:,jj,kk,2,nn)); m_sz(:,jj,kk,nn) = squeeze(m_si(:,jj,kk,3,nn));
            m_si_unit(:,jj,kk,1,nn) = m_sx(:,jj,kk,nn)./(sqrt(m_sx(:,jj,kk,nn).^2+m_sy(:,jj,kk,nn).^2+m_sz(:,jj,kk,nn).^2));
            m_si_unit(:,jj,kk,2,nn) = m_sy(:,jj,kk,nn)./(sqrt(m_sx(:,jj,kk,nn).^2+m_sy(:,jj,kk,nn).^2+m_sz(:,jj,kk,nn).^2));
            m_si_unit(:,jj,kk,3,nn) = m_sz(:,jj,kk,nn)./(sqrt(m_sx(:,jj,kk,nn).^2+m_sy(:,jj,kk,nn).^2+m_sz(:,jj,kk,nn).^2));
            
            m_fi(:,jj,kk,:,nn) = reshape(cross(squeeze(allnvec_mat(:,jj,kk,:,nn)),squeeze(m_si_unit(:,jj,kk,:,nn)),2),[xy 1 1 3]);
            m_fx(:,jj,kk,nn) = squeeze(m_fi(:,jj,kk,1,nn)); m_fy(:,jj,kk,nn) = squeeze(m_fi(:,jj,kk,2,nn)); m_fz(:,jj,kk,nn) = squeeze(m_fi(:,jj,kk,3,nn));
            m_fi_unit(:,jj,kk,1,nn) = m_fx(:,jj,kk,nn)./(sqrt(m_fx(:,jj,kk,nn).^2+m_fy(:,jj,kk,nn).^2+m_fz(:,jj,kk,nn).^2));
            m_fi_unit(:,jj,kk,2,nn) = m_fy(:,jj,kk,nn)./(sqrt(m_fx(:,jj,kk,nn).^2+m_fy(:,jj,kk,nn).^2+m_fz(:,jj,kk,nn).^2));
            m_fi_unit(:,jj,kk,3,nn) = m_fz(:,jj,kk,nn)./(sqrt(m_fx(:,jj,kk,nn).^2+m_fy(:,jj,kk,nn).^2+m_fz(:,jj,kk,nn).^2));
        end
    end
end
toc
