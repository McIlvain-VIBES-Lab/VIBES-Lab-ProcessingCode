function [Brfo_all,Bifo_all,X,Y,Z, coef,BulkMotion]=calc_3Ddisp_BulkMotionRemoved_DRS(Xmotion,Ymotion,Zmotion)
% written by RJO on 2/22/13
% Edited by DR Smith 4/23/20
...
Brf_all = real(cat(4,Xmotion,Ymotion,Zmotion));
Bif_all = imag(cat(4,Xmotion,Ymotion,Zmotion));
if nargin < 2,
 ...
 [X Y Z] = meshgrid(1:size(Brf_all,2), 1:size(Brf_all,1),1:size(Brf_all,3));
    ...
    BulkMotion=predmat;
save('BMR.mat','Brfo_all','Bifo_all','BulkMotion')
end

%%
%{
First, load the XYZ motion data from whichever data set you want to
analyze.
Then run this in the command prompt "calc_3Ddisp_BulkMotionRemoved(Xmotion,Ymotion,Zmotion)"
Load the newly saved file (BMR.mat) and run "buckytwo"
%}