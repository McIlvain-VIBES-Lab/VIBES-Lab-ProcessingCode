function combo_2waves_XBOX(directions,threshold,waves)
...
elseif directions == 2
    ...
    Uperctot = cat(3,UperctotAP,UperctotLR); Mutotlist = cat(1,MutotlistAP,MutotlistLR);
    Amptotlist = cat(3,AmplistAP,AmplistLR); Thlist = cat(3,ThlistAP,ThlistLR);
elseif directions == 3
    cd LR; load Quant_MUFS_LR.mat; cd ..
...
%for ti = 1:13;
    ...
    clear Atot
        ...
        FAv = FAtractlistAP(:)>FA_thresh;
        for wv = 1:waves
            for dirs = 1:length(Mutotlist(:,1))
                clear Sv Fv AS AF MuS MuF v Mu MuT
                
                Sv = Uperctot(:,(2*wv-1),dirs)>SF_thresh; Fv = Uperctot(:,(2*wv),dirs)>SF_thresh;
                MuS = permute(Mutotlist(dirs,logical(Sv.*FAv)),[2 1]); MuF = permute(Mutotlist(dirs,logical(Fv.*FAv)),[2 1]);
                v = (Sv+Fv)>0;
%                 sum(Sv)
%                 sum(Fv)
%                 sum(v)
                MuT = permute(Mutotlist(dirs,:),[2 1]); Mu = MuT(logical((MuT>0).*FAv.*v));
                
                AS(1:length(MuS),1) = 1;
                AS(1:length(MuS),2) = cos(Thlist(logical(Sv.*FAv),wv,dirs)).^2;
                AS(1:length(MuS),3) = 0;
                size(AS)
                AF(1:length(MuF),1) = 1;
                AF(1:length(MuF),2) = cos(2*Thlist(logical(Fv.*FAv),wv,dirs)).^2;
                AF(1:length(MuF),3) = sin(2*Thlist(logical(Fv.*FAv),wv,dirs)).^2;
                size(AF)
                
                AmpS = Amptotlist(logical(Sv.*FAv),wv,dirs);
                AmpF = Amptotlist(logical(Fv.*FAv),wv,dirs);
                
                Arep = cat(1,(AS.*repmat(squeeze(Uperctot(logical(Sv.*FAv),(2*wv-1),dirs)),[1 3])/100),(AF.*repmat(squeeze(Uperctot(logical(Fv.*FAv),(2*wv),dirs)),[1 3]))/100);
                size(Arep)
