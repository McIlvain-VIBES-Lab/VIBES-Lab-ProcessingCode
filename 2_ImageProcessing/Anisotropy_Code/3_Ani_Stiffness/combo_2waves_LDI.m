function [x_soln] = combo_2waves_LDI(directions,threshold,waves)
...
if directions == 1
    ...
    crit = critAP;
elseif directions == 2
    cd LR; load Quant_MUFS_LRall.mat; cd ..
    ...
    clear Atot
        ...
        for wv = 1:waves
            for dirs = 1:length(Mutotlist(1,1,1,:))
                ...
                Musx = Mutotlist(:,ti,2*wv-1,dirs); Mufx = Mutotlist(:,ti,2*wv,dirs);
                Mus = Musx(logical((Musx>0).*FAv)); Muf = Mufx(logical((Mufx>0).*FAv));
                
                AS(1:length(Mus),1,1) = 1;
                AS(1:length(Mus),1,2) = cos(Thlist(logical(FAv),wv,ti,dirs)).^2;
                AS(1:length(Mus),1,3) = 0;
                size(AS)
                
                AF(1:length(Muf),1,1) = 1;
                AF(1:length(Muf),1,2) = cos(2*Thlist(logical(FAv),wv,ti,dirs)).^2;
                AF(1:length(Muf),1,3) = sin(2*Thlist(logical(FAv),wv,ti,dirs)).^2;
                size(AF)
