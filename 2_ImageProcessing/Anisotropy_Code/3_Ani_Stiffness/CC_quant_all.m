...
%% CC Tracts (1 - Body, 2 - Genu, 3 Splenium, 4 - all)
...
for ii = 1:4
    vox = zeros(size(Mus1));
    TR = CC(:,:,:,ii); 
    maskx = TR.*DTI.*maskAP.*maskLR;
    list = maskx(maskx==1);
    
    %% Mu
    nn = Mus1.*maskx; nn(isnan(nn)) = 0; figure;im(nn>0)
...
save('Quant_MUFS_APall.mat','TotalAP','MutotlistAP','ThlistAP','FAtractlistAP','critAP','GplistAP')
% cd ..
%% LR
cd ..; cd LR
%addpath('PostInv')
...
load Mu.mat
% cd LDI_compiled; load Mu_LDI_diff.mat
load Mu_LDI_diff414.mat
...
save('Quant_MUFS_LRall.mat','TotalLR','MutotlistLR','ThlistLR','FAtractlistLR','critLR','GplistLR')
