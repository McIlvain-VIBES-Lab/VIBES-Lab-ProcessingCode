  function [Mus1,Muf1,Mus2,Muf2,Gp,Gdp] = skel_filter3(x,y,z,V1index,V2index,mask,t2stack,dtiall)
tic
[ny, nx, nz, ~] = size(V1index);
ni = 3;
cd 300filt

filt1 = V1index(y,x,z); filt2 = V2index(y,x,z);
tmp1 = load(sprintf('U_filt%d.mat',filt1)); U_filt1 = tmp1.Ufilt;
tmp2 = load(sprintf('U_filt%d.mat',filt2)); U_filt2 = tmp2.Ufilt;
cd ..
...
Mus1 = abs(Gps1(x-9,y-9,z)+1i*Gdps1(x-9,y-9,z));
Gps1x = Gps1(x-9,y-9,z);
Gdps1x = Gdps1(x-9,y-9,z);
...
Muf1 = abs(Gpf1(x-9,y-9,z)+1i*Gdpf1(x-9,y-9,z));
Gpf1x = Gpf1(x-9,x-9,z);
Gdpf1x = Gdpf1(x-9,y-9,z);
...
Mus2 = abs(Gps2(x-9,y-9,z)+1i*Gdps2(x-9,y-9,z));
Gps2x = Gps2(x-9,x-9,z);
Gdps2x = Gdps2(x-9,y-9,z);
...
Muf2 = abs(Gpf2(x-9,y-9,z)+1i*Gdpf2(x-9,y-9,z));
Gpf2x = Gpf2(x-9,y-9,z);
Gdpf2x = Gdpf2(x-9,y-9,z);
