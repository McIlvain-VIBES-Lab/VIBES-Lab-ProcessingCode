...
F_vox = sum(Nf_norm,4)>sum(Ns_norm,4); S_vox = sum(Ns_norm,4)>sum(Nf_norm,4);

Ns_normx = Ns_norm(:,:,:,1); Ns_normy = Ns_norm(:,:,:,2); Ns_normz = Ns_norm(:,:,:,3);
Ns_norm_unit(:,:,:,1) = Ns_normx./sqrt(Ns_normx.^2+Ns_normy.^2+Ns_normz.^2);
Ns_norm_unit(:,:,:,2) = Ns_normy./sqrt(Ns_normx.^2+Ns_normy.^2+Ns_normz.^2);
Ns_norm_unit(:,:,:,3) = Ns_normz./sqrt(Ns_normx.^2+Ns_normy.^2+Ns_normz.^2);

Nf_normx = Ns_norm(:,:,:,1); Nf_normy = Nf_norm(:,:,:,2); Nf_normz = Nf_norm(:,:,:,3);
Nf_norm_unit(:,:,:,1) = Nf_normx./sqrt(Nf_normx.^2+Nf_normy.^2+Nf_normz.^2);
Nf_norm_unit(:,:,:,2) = Nf_normy./sqrt(Nf_normx.^2+Nf_normy.^2+Nf_normz.^2);
Nf_norm_unit(:,:,:,3) = Nf_normz./sqrt(Nf_normx.^2+Nf_normy.^2+Nf_normz.^2);



%% Solve for Thetas
Theta_slow = abs(acosd(dot(Ns_norm_unit,A,4)));
Theta_slow(Theta_slow==90) = NaN;
Theta_slow = Theta_slow -(Theta_slow>180)*180; 
Theta_fast = acosd(dot(Nf_norm_unit,A,4));
Theta_fast(Theta_fast==90) = NaN;
Theta_fast = Theta_fast -(Theta_fast>180)*180; 