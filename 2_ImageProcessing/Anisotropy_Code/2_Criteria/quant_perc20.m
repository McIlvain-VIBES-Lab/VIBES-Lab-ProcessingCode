%% Load in AP/LR/SI files from their seperate locations
% Change Locations to AP and LR folders
cd /Volumes/CLJ-001/Group/drsmitty/ScanData/Multi/170816/Clj_Multiexcite_04_02/MRE/AP
load Buckyout.mat
load slowfasttwo.mat
load Multiexcite_04_02_AP_170816.mat
[percent20_AP,Unorms_AP,perc20_AP] = percent(A1x,A2x,U_s_V1,U_f_V1,U_s_V2,U_f_V2,mask);
U_s = U_s_V1+U_s_V2;
totalvox = sum(sum(sum(abs(U_s)>0)));
V1_AP = V1x; V2_AP = V2x;

cd /Volumes/CLJ-001/Group/drsmitty/ScanData/Multi/170816/Clj_Multiexcite_04_02/MRE/LR
load Buckyout.mat
load slowfasttwo.mat
load Multiexcite_04_02_LR_170816.mat
[percent20_LR,Unorms_LR,perc20_LR] = percent(A1x,A2x,U_s_V1,U_f_V1,U_s_V2,U_f_V2,mask);
V1_LR = V1x; V2_LR = V2x;

cd /Volumes/CLJ-001/Group/drsmitty/ScanData/Multi/170816/Clj_Multiexcite_04_02/MRE/SI
load Buckyout.mat
load slowfasttwo.mat
load Multiexcite_04_02_SI_170816.mat
[percent20_SI,Unorms_SI,perc20_SI] = percent(A1x,A2x,U_s_V1,U_f_V1,U_s_V2,U_f_V2,mask);
V1_SI = V1x; V2_SI = V2x;

cd /Volumes/CLJ-001/Group/drsmitty/ScanData/Multi/170816/Clj_Multiexcite_04_02/Tracts/WMmain
...
cd /Volumes/CLJ-001/Group/drsmitty/ScanData/Multi/170816/Clj_Multiexcite_04_02/Tracts/WMminor/niiTracts
...
VslowSI1 = Us1perc20_SI+Us2perc20_SI; VfastSI1 = Uf1perc20_SI+Uf2perc20_SI;
Vslowall_c = V1s_AP+V1s_LR+V2s_AP+V2s_LR; Vfastall_c = V1f_AP+V1f_LR+V2f_AP+V2f_LR;
Vslowall_c_wSI = V1s_AP+V1s_LR+V1s_SI+V2s_AP+V2s_LR+V2s_SI; Vfastall_c_wSI = V1f_AP+V1f_LR+V1f_SI+V2f_AP+V2f_LR+V2f_SI;
Vall_c = Vslowall_c+Vfastall_c;
Vmult = (Vslowall_c>=2).*(Vfastall_c>=2);
Vmult_wSI = (Vslowall_c_wSI>=2).*(Vfastall_c_wSI>=2);
...
VAPslowall = VslowAP1; VAPfastall = VfastAP1; 
VLRslowall = VslowLR1; VLRfastall = VfastLR1; VSIslowall = VslowSI1; VSIfastall = VfastSI1; 
...
VLRslowCCb = VslowLR1.*CCb; VLRfastCCb = VfastLR1.*CCb; VSIslowCCb = VslowSI1.*CCb; VSIfastCCb = VfastSI1.*CCb; 
...
VLRslowCC = VslowLR1.*CC; VLRfastCC = VfastLR1.*CC; VSIslowCC = VslowSI1.*CC; VSIfastCC = VfastSI1.*CC; 
VAPpercCC = sum(sum(sum((VAPslowCC>=2).*(VAPfastCC>=2))))/totalvoxCC;
VLRpercCC = sum(sum(sum((VLRslowCC>=2).*(VLRfastCC>=2))))/totalvoxCC;
% VSIpercCC = sum(sum(sum((VSIslowCC>=2).*(VSIfastCC>=2))))/totalvoxCC;
...
VLRslowCR = VslowLR1.*CR; VLRfastCR = VfastLR1.*CR; VSIslowCR = VslowSI1.*CR; VSIfastCR = VfastSI1.*CR; 
VAPpercCR = sum(sum(sum((VAPslowCR>=2).*(VAPfastCR>=2))))/totalvoxCR;
VLRpercCR = sum(sum(sum((VLRslowCR>=2).*(VLRfastCR>=2))))/totalvoxCR;
% VSIpercCR = sum(sum(sum((VSIslowCR>=2).*(VSIfastCR>=2))))/totalvoxCR;
...
VLRslowCCS = VslowLR1.*CCS; VLRfastCCS = VfastLR1.*CCS; VSIslowCCS = VslowSI1.*CCS; VSIfastCCS = VfastSI1.*CCS; 
VAPpercCCS = sum(sum(sum((VAPslowCCS>=2).*(VAPfastCCS>=2))))/totalvoxCCS;
VLRpercCCS = sum(sum(sum((VLRslowCCS>=2).*(VLRfastCCS>=2))))/totalvoxCCS;
% VSIpercCCS = sum(sum(sum((VSIslowCCS>=2).*(VSIfastCCS>=2))))/totalvoxCCS;
...
VLRslowSLF = VslowLR1.*SLF; VLRfastSLF = VfastLR1.*SLF; VSIslowSLF = VslowSI1.*SLF; VSIfastSLF = VfastSI1.*SLF; 
VAPpercSLF = sum(sum(sum((VAPslowSLF>=2).*(VAPfastSLF>=2))))/totalvoxSLF;
VLRpercSLF = sum(sum(sum((VLRslowSLF>=2).*(VLRfastSLF>=2))))/totalvoxSLF;
% VSIpercSLF = sum(sum(sum((VSIslowSLF>=2).*(VSIfastSLF>=2))))/totalvoxSLF;
...
save('SlowFastSummation.mat','all_all','Uall','TW_cps','TW_tracts','CCb_all','CC_all','CR_all','CCS_all','SLF_all','Vslow','Vfast','Uslowall','Ufastall','Vmult','Vmult_wSI')
