function dti_proc_rotatebvecs_CALF
...
!$FSLDIR/bin/flirt -in dti_S0.nii -ref t2bet.nii -out dti2mre.nii -omat dti2mre.mat -dof 6
...
!$FSLDIR/bin/flirt -in DTI.nii -ref t2bet.nii -o Diff_rotated.nii -applyxfm -init dti2mre.mat -dof 6
...
!$FSLDIR/bin/bet2 dti_rot_b0.nii dti -f 0.15 -w 1.5 -m -n -v
!mv dti_mask.nii.gz dti_rot_mask.nii.gz
!gunzip -f dti_rot_mask.nii.gz

tmp2 = load_untouch_nii('dti_rot_mask.nii');

tmp1 = load_untouch_nii('dti_rot_b0.nii');
tmp = tmp1.img;
mask_med = zeros(size(tmp));
for ii = 1:20
    mask_med(:,:,ii) = medfilt2(squeeze(tmp(:,:,ii)),[8 8]);
end
mask_med1 = mask_med>(5); %change number here
mask_test = cat(3,ones([80 80 1]),mask_med1,ones([80 80 1]));
mask_fill = imfill(mask_test,'holes');
mask_fill(:,:,22) = []; mask_fill(:,:,1) = [];
tmp2.img = mask_fill;
save_untouch_nii(tmp2,'dti_rot_mask1.nii')


%!$FSLDIR/bin/flirt -in dti_mask.nii -ref t2bet.nii -o dti_mask_new.nii -applyxfm -init dti2mre.mat -dof 6
%!gunzip -f dti_mask_new.nii.gz

!$FSLDIR/bin/dtifit -k Diff_rotated.nii -o dti_rot -m dti_rot_mask1.nii -r Diff_rotated.bvec -b Diff.bval -V
