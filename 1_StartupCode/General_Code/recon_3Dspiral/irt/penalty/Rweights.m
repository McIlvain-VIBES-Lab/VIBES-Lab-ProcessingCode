...
case 'leak'
	...
	if order == 1
		...
		if 1 % handle edge effects properly!
			...
			dd = penalty_displace(offset, Nd);
			good = (ic - in) == repmat(dd, [length(ii) 1]);
...
case 'tight'
	...
	if order == 1
		...
		if 1 %ndle edge effects properly!
			ic = pn.ind2sub(Nd, ii);
			in = pn.ind2sub(Nd, ii-offset);
			dd = penalty_displace(offset, Nd);
			good = (ic - in) == repmat(dd, [length(ii) 1]);
...
for order=1:2
 for ie=1:length(edge_types)
	...
	printm('order=%d edge_type=%s', order, edge_type)
	...
	if ~streq(edge_type, 'simple') && has_mex_jf
		...
		if max_percent_diff(wt, wx) > 1e-5
			im clf, im pl 1 2
			im(1, ig.shape(wt), 'mat')
			im(2, ig.shape(wx), 'mex')
			fail('mat vs mex bug')
		end
	end
	...
	for mm=1:ncol(wt)
		if ~isequal(wp.col(mm), wt(:,mm)), error 'wp bug', end
		if ~isequal(ws.col(mm), wt(:,mm)), error 'ws bug', end
