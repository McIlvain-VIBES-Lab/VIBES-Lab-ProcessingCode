  function [wmap, wconv] = mri_field_map_reg2(yik, etime, varargin)
...
totalsum(isnan(totalsum)) = 0;   % BPS ADDED 
wjfactor = sqrt(wjfactor ./ totalsum);
wjfactor(isnan(wjfactor)) = 0;   % BPS ADDED

totalsum2 = (abs(yik(:,2)).^2 + abs(yik(:,1)).^2) ./ (abs(yik(:,1)).^2);
totalsum2(isnan(totalsum2)) = 0;   % BPS ADDED 
wjfactor2 = sqrt(wjfactor2 ./ totalsum2);
wjfactor2(isnan(wjfactor2)) = 0;   % BPS ADDED

% Scale data by this factor
yi2 = [yik(:,1) ./ wjfactor2, yik(:,2) ./ wjfactor2];
yi2(isnan(yi2)) = 0;  % BPS ADDED
for j=1:nset
	yik(:,j) = yik(:,j) ./ wjfactor;
end
yik(isnan(yik)) = 0;  % BPS ADDED
