...
if istart ~= 1 || nblock ~= 1
	...
	cg.na = length(ia);
	com = [com sprintf(' na %d orbit %g orbit_start %g source_z0 %g', ...
		cg.na, cg.orbit, cg.orbit_start, cg.source_z0)]; % trick
end
...
if is_transpose % back
	...
	for ll=1:LL
		...
		tmp = os_run(com);
		x(:,:,:,ll) = fld_read(arg.hct2.x, 'raw', 1);
	end
	x = flipper3(x, arg);
...
%		eval(['!' com])
		tmp = os_run(com);
		y(:,:,:,ll) = fld_read(arg.hct2.y, 'raw', 1);
	end
