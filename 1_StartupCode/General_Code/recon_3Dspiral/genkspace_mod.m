function [kx,ky,gx,gy] = genkspace_mod(FOV,N,ld,nint,gamp,gslew,tsamp,rotamount,rev_flag,gts,flag_vd,int_rotation,alpha_vd, zoomfact, n_int_fact);
...
%dts = 4e-6;    %5e-6
if (flag_vd == 1)
  [Gx,Gy,kxi,kyi,sx,sy] = genspivd_Kim_mod(FOV,N,nint,gamp,gslew,gts,alpha_vd,zoomfact);
else
  [Gx,Gy,kxi,kyi,sx,sy] = genspi(FOV,N,zoomfact*nint,gamp,gslew,gts);
end
...
sprintf('Performing %d times %d rotations',nint, n_int_fact)
%kx(:,1) = kxop;
%ky(:,1) = kyop;
phi = 2*pi/(n_int_fact*nint);

for ii = 0:(n_int_fact*nint-1)
     ang_rot = phi*flag_sign*(ii*int_rotation - (nint*n_int_fact-1)*floor(ii*int_rotation/(nint*n_int_fact)));
