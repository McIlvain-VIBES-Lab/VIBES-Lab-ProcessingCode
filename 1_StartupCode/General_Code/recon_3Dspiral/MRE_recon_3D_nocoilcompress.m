function MRE_recon_3D_nocoilcompress(recon_type,nc_rank)
% This is the recon code for MRE in the INSIGHT project:
% - simply a specific version of MRE_recon_3D_2014_08_07_v08
% - 7/24 version deals with the switch to parpool from matlabpool
    ...
    for ii = 1:nslab
        ...
        for kk = (ntp_start+dummies):ntp_recon
                parfor ii=1:nslab
                    ...
                    S = sense(A ,sen_tmp(logical(mask_tmp),:)); % S = sense(A ,sen_map);
                    t_img = pwls_pcg1(imginit(logical(mask_tmp)), S, 1,col(curvol), R, 'niter', niter);
...
function msk = data_mask(msk_on, slab_num, N, nsl, folder, N_recon, nsl_recon)
    if msk_on
        ...
        msk = col(ones(N_recon, N_recon, nsl_recon));
    end
    clear mask
