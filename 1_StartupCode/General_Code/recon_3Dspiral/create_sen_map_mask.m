function sen = create_sen_map_mask(recoInfo,flag_average)
% function sen = create_sen_map(recoInfo,flag_average)
%
% create sensitivity maps from multicoil images
% from BPS recon, new version modified by JLH 9/2012
% edited by CLJ for performance with other codes
%
% Inputs
...
beta = 1e-5;
niter = 1000;
init_img = 1e-2*ones(nx*nx,1);
Cn = C3D_sparse(ones(nx,nx))*beta;
sen = zeros(nx,nx,nz,num_coils);
for sliceIndex = 1:nz
    A = sensemult(mask(:,:,sliceIndex).*sos_img(:,:,sliceIndex));
    for coilIndex = 1:num_coils
        tmp = qpwls_pcg_largeIter(init_img, A, 1,col(mask(:,:,sliceIndex).*coil_imgs(:,:,sliceIndex,coilIndex)), 0, Cn, 1, niter);
        sen(:,:,sliceIndex,coilIndex) = reshape(tmp,nx,nx);
    end
end
