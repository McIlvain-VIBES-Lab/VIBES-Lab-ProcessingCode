...
%     args = {[2*pi/N1*ky,2*pi/N2*kx, 2*pi/N3*kz],[N1,N2, N3],[Jm1,Jm2,Jm3],[K1,K2,K3],[Nx/2-1/2,Ny/2-1/2, Nz/2-1/2], 'linear', 2^10}; 
    ...
    if gridflag == 2
        args = {[2*pi/N1*ky,2*pi/N2*kx],[N1,N2],[Jm1,Jm2],[K1,K2],[N1/2,N2/2]*(1), 'table', 2^10, 'minmax:kb'};
        warning('Using table lookup...');
    else
        args = {[2*pi/N1*ky,2*pi/N2*kx],[N1,N2],[Jm1,Jm2],[K1,K2],[N1/2,N2/2]*(1), 'minmax:kb', 2^10};
    end
